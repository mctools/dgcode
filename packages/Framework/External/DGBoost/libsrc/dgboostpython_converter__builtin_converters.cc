...
#include <dgboost/python/detail/wrap_python.hpp>
...
// when long long is absent but __int64 is present.
  ...
  struct wstring_rvalue_from_python
  {
      ...
      static std::wstring extract(PyObject* intermediate)
      {
          ...
          if (buf == NULL) {
              dgboost::python::throw_error_already_set();
          }
...
{
#if PY_VERSION_HEX >= 0x03000000
    return x ? PyUnicode_FromString(x) : dgboost::python::detail::none();
#else
    return x ? PyString_FromString(x) : dgboost::python::detail::none();
#endif
}
  
BOOST_PYTHON_DECL PyObject* do_return_to_python(PyObject* x)
{
    return x ? x : dgboost::python::detail::none();
}
  
BOOST_PYTHON_DECL PyObject* do_arg_to_python(PyObject* x)
{
    if (x == 0)
        return dgboost::python::detail::none();

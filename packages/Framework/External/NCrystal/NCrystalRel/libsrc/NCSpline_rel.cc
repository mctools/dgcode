...
void NCrystalRel::PiecewiseLinearFct1D::dumpToFile( const std::string& filename ) const {
  std::ofstream ofs (filename.c_str(), std::ofstream::out);
  ofs << std::setprecision(20);
  ofs << "#colnames=x,y\n";
  ofs << "#plotstyle=*-\n";
...
double NCrystalRel::PiecewiseLinearFct1D::evalEdgeCase( VectD::const_iterator it, double x ) const
{
  if ( it == m_x.end() ) {
    if ( !m_ofVals.overflowYValue.has_value() )

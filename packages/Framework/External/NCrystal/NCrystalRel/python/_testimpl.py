...
def _actual_test_cmdline( verbose ):
    ...
    cmds = ['ncrystal-config --help',
            ...
            'nctool --help',
            'nctool --test',
            'ncrystalrel_inspectfile --help',
            'ncrystalrel_endf2ncmat --help',
            'ncrystalrel_hfg2ncmat --help',
            'ncrystalrel_ncmat2cpp --help',
            'ncrystalrel_ncmat2hkl --help',
            'ncrystalrel_cif2ncmat --help',
            'ncrystalrel_vdos2ncmat --help',
            'ncrystalrel_verifyatompos --help',
...
def _actual_test_cmake( verbose = False, ignore_if_absent = False ):
    ...
    ok, ncrystalrel_cmakedir = _run_cmd(_cmd)
    if not ok:
        raise RuntimeError('Command failed: %s'%_cmd)
    ncrystalrel_cmakedir = _singleline2str(ncrystalrel_cmakedir)
    prfct('ncrystal-config gives cmakedir: %s'%ncrystalrel_cmakedir)
    _cmd = 'ncrystal-config --show libdir'
    ok, ncrystalrel_libdir = _run_cmd(_cmd)
    if not ok:
        raise RuntimeError('Command failed: %s'%_cmd)
    ncrystalrel_libdir = _singleline2str(ncrystalrel_libdir)
    prfct('ncrystal-config gives libdir: %s'%ncrystalrel_libdir)
    ...
    prepend_to_path( cmake_env, 'CMAKE_PREFIX_PATH', ncrystalrel_cmakedir )
    prepend_to_path( runtime_env, 'LD_LIBRARY_PATH', ncrystalrel_libdir )
    prepend_to_path( runtime_env, 'DYLD_LIBRARY_PATH', ncrystalrel_libdir )
    def runcmake(args):
...
#include <iostream>
int main() {
  auto pc = NCrystalRel::createScatter( \\"Al_sg225.ncmat;dcutoff=0.5;temp=25C\\" );
  auto wl = NCrystalRel::NeutronWavelength{1.8};
...
target_link_libraries( exampleapp NCrystalRel::NCrystal )

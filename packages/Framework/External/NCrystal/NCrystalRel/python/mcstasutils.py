...
"""
    ...
    for proccfgstr, proctypename in physlist:
        if len(physlist)==1 and proctypename=='total':
            procname=f'{name}_ncrystalrel_proc'
        else:
            procname=f'{name}_ncrystalrel_{proctypename}_proc'
...
def cfgstr_2_hkl(*, cfgstr, tgtformat, verbose=True, fp_format = '%.14g', prefer_systemexit_exceptions = False ):
    ...
    yield '#'
    yield f'# ncrystalrel_cfgstr "{cfgstr}"'
    yield '#'
    if tgtformat=='laz':
    ...
    if cfgstr_nodataname:
        yield '#'
        yield f'# ncrystalrel_embedded_cfg : NCRYSTALMATCFG[{cfgstr_nodataname}]'
        yield '#'

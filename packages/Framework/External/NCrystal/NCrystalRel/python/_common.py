...
def set_ncrystalrel_print_fct( fct ):
    _stdprint[0] = fct
    return print

def get_ncrystalrel_print_fct():
    return _stdprint[0]

def warn(msg):
    """Emit NCrystalUserWarning via standard warnings.warn function"""
    from .exceptions import NCrystalRel as NCrystalUserWarning
    import warnings
...
def prettyFmtValue(x):
    ...
    but can also reinject accuracy in atom positions (ncrystalrel_verifyatompos can

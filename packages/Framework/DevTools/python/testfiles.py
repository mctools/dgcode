...
ignore_dirs = [cfg.dirs.blddir, cfg.dirs.installdir, *[d / '.git' for d in topdirs], normpath(pathlib.Path(os.environ['DGCODE_DIR'])/'packages'/'Projects'/'FmwkAttic'/'Validation'/'DynPkgs')]
...
for f in all_files():
    ...
    if f.is_dir():
        ...
        if f == cfg.dirs.projdir/'bootstrap.sh':
          continue
        if f.name == 'LICENSE':
            #must be in pkg directory or in non-pkg dir under a dir named 'External':
            if f.parent in all_pkgdirs:
                continue
            if f.parent.parent.name == 'External':
                continue

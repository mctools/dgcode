...
def _get_custom():
    ...
    _custom={}
    import glob,os
    for mod in glob.glob(os.path.join(os.getenv('ESS_INSTALL_PREFIX'),'python/*/g4physlist_*.so')):
        bn=os.path.splitext(os.path.basename(mod))[0]
        packagename = os.path.basename(os.path.dirname(mod))
        physlistname = custom_name_pattern%bn[11:]

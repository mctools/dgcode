...
void G4DataCollectInternals::DBElementEntry::write(EvtFile::FileWriter&fw)
{
  ...
  fw.writeDataDBSection(uint32_t(const_cast<G4Element*>(m_elem)->GetNaturalAbundanceFlag()?1:0));
#else
  fw.writeDataDBSection(uint32_t(const_cast<G4Element*>(m_elem)->GetNaturalAbandancesFlag()?1:0));
#endif
  //ignoring atomic shell info
  uint32_t niso = m_elem->GetNumberOfIsotopes();

...
shquote = lambda s : _shlex_quote(str(s) if hasattr(s,'__fspath__') else s)

import dgbuild.cfg as dgcfg

assert len(sys.argv)==2,"Please specify location of unpacked boost source tar-ball"
boost_src = sys.argv[1]
dgdir = pathlib.Path(os.environ['DGCODE_FMWK_DIR'])
dgsysmodsdir = dgdir / '.system/mods'
assert dgsysmodsdir.exists()
sys.path = [str(dgsysmodsdir)] + sys.path
tmpdir = pathlib.Path(os.environ['DGCODE_BUILD_DIR_RESOLVED']) / 'tmpboost'
import utils
inctarget = dgdir / '.system/include/dgboost'
...
block_header_mods = 'atomic chrono date_time dynamic_bitset fusion intrusive mpi program_options proto random ratio regex spirit test thread timer xpressive io'
...
for lm in sorted(lib_mods.keys()):
    ...
    if pkgdeps is not None:
        pkgdir = pathlib.Path(dgcfg.pkgs['DGBoost']['dirname'])
        ...
        pkgsrc = pathlib.Path(dgcfg.pkgs['DGBoost']['dirname'])/'libsrc'
        #pkgsrc=os.path.join(dgdir,'packages/Framework/External/DGBoost/libsrc')
        for oldfile in glob.glob(os.path.join(str(pkgsrc),'dgboost*')):
            os.remove(oldfile)

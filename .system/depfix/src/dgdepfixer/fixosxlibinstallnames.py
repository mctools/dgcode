...
def set_install_names_to_abs_path(libdir,prefix='',verbose=False):
    ...
    libdir=os.path.abspath(os.path.realpath(os.path.expanduser(str(libdir))))
    ...
    # via interpreters in system dirs (including '#!/usr/bin/env python' and
    # #!/bin/bash).
    ...
    for rbn,lib in sorted(rbn2lib.items()):
        ...
        tmp = DG.sysutils.system_throw("otool -D %s"%shlex.quote(absname),True).decode()
        lib.install_name_orig = tmp.splitlines()[1].strip()
    ...
    for rbn,lib in sorted(rbn2lib.items()):
        absname=os.path.join(libdir,rbn)
        tmp = DG.sysutils.system_throw("otool -L %s"%shlex.quote(absname),True).decode().splitlines()
    ...
    for rbn,lib in sorted(rbn2lib.items()):
        ...
        if verbose:
            print("%sTreating library %s:"%(prefix,shlex.quote(afn)))
        if not lib.install_name_changed() and not lib.deps:
            if verbose:
                print("%sNothing to do"%prefix)
            else:
                print('%sChanged 0 install names in %s'%(prefix,shlex.quote(afn)))
            continue
        if lib.install_name_changed():
            cmd='install_name_tool -id %s %s'%(shlex.quote(lib.install_name_new),shlex.quote(afn))
            if verbose:
                print("%s   %s"%(prefix,cmd))
            nfix+=1
            DG.sysutils.system_throw(cmd)
        for d in lib.deps:
            cmd='install_name_tool -change %s %s %s'%(shlex.quote(d),shlex.quote(instname_map[d]),shlex.quote(afn))
            if verbose:
                print("%s   %s"%(prefix,cmd))
            nfix+=1
            DG.sysutils.system_throw(cmd)
        if not verbose:
            print('%sChanged %i install names in %s'%(prefix,nfix,shlex.quote(afn)))

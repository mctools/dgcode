...
swversions={
    ...
    'CMake': dict(oldest='3.15.3',recommended='3.24.2',newest=None),#v3.8.2 supports C++17, v3.12.4 supports FindPython3 and C++20. v3.12.3 comes with Conda, we hope it is good enough. v3.15 has improved search policies for python.
    ...
    'pymod:matplotlib':dict(oldest='3.1.3'),#3.0.0 is first release without python2 support. NB: DMSC with 3.5 only supports up to 3.0.3 (THIS COMMENT IS OUTDATED DMSC NOW HAVE 3.6). NB: Matplotlib 3.2.1 introduced dependency on cm-super font, which dgdepfixer would need to look for!!! See also DGSW-520 before updating recommended version.
    'pymod:ptpython':dict(oldest='2.0.4',recommended='3.0.20'),#NB: Keep a recommended version for ptpython now  (see special ptpython treatment in platformutils.py)

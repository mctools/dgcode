...
//      great as dgboost::intmax_t can cause some differences to be
...
namespace dgboost {} namespace boost = dgboost; namespace dgboost { namespace detail {
  ...
  struct digit_traits<T, false>
  {
      BOOST_STATIC_CONSTANT(int, digits = (
          ...
          - (dgboost::is_signed<T>::value ? 1 : 0))
          );
  };
#endif

  // Template class integer_traits<Integer> -- traits of various integer types
  // This should probably be rolled into dgboost::integer_traits one day, but I
      ...
      typedef typename dgboost::conditional<
        (int(x::is_signed)
          && (!int(x::is_bounded)
             ...
             || (int(x::digits) + 1 >= digit_traits<dgboost::intmax_t>::digits))),
        Integer,
...
#else
      BOOST_STATIC_ASSERT(dgboost::is_integral<Integer>::value);

      typedef typename dgboost::conditional<
        (sizeof(Integer) >= sizeof(intmax_t)),

        dgboost::conditional<
          (dgboost::is_signed<Integer>::value),
          Integer,
          dgboost::intmax_t
        >,

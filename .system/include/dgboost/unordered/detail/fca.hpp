...
#include <dgboost/unordered/detail/prime_fmod.hpp>

#include <dgboost/core/addressof.hpp>
#include <dgboost/core/allocator_access.hpp>
#include <dgboost/core/bit.hpp>
#include <dgboost/core/empty_value.hpp>
  ...
  namespace unordered {
    namespace detail {
      ...
      template <class ValueType, class VoidPtr> struct node
      {
        ...
        typedef typename dgboost::pointer_traits<VoidPtr>::template rebind_to<
          node>::type node_pointer;

        node_pointer next;
        typename dgboost::aligned_storage<sizeof(value_type),
          dgboost::alignment_of<value_type>::value>::type buf;
      ...
      template <class Bucket> struct bucket_group
      {
        ...
        typedef
          typename dgboost::pointer_traits<bucket_pointer>::template rebind_to<
      ...
      public:
        ...
        typedef typename dgboost::pointer_traits<bucket_pointer>::difference_type
          difference_type;
      ...
      private:
        ...
        void increment() BOOST_NOEXCEPT
        {
          ...
          std::size_t n = std::size_t(dgboost::core::countr_zero(
          ...
          if (n < N) {
            ...
            std::ptrdiff_t x = dgboost::core::countr_zero(pbg->bitmask);
            p = pbg->buckets + x;
          }
        }
      };
      ...
      class grouped_bucket_array
          : dgboost::empty_value<typename dgboost::allocator_rebind<Allocator,
              node<typename dgboost::allocator_value_type<Allocator>::type,
                typename dgboost::allocator_void_pointer<Allocator>::type> >::
                type>
      {
        BOOST_MOVABLE_BUT_NOT_COPYABLE(grouped_bucket_array)

        typedef typename dgboost::allocator_value_type<Allocator>::type
          allocator_value_type;
        typedef
          typename dgboost::allocator_void_pointer<Allocator>::type void_pointer;
        typedef typename dgboost::allocator_difference_type<Allocator>::type
          difference_type;

      public:
        typedef typename dgboost::allocator_rebind<Allocator,
          node<allocator_value_type, void_pointer> >::type node_allocator_type;

        typedef node<allocator_value_type, void_pointer> node_type;
        typedef typename dgboost::allocator_pointer<node_allocator_type>::type
          node_pointer;
        typedef SizePolicy size_policy;

      private:
        typedef typename dgboost::allocator_rebind<Allocator, Bucket>::type
          bucket_allocator_type;
        typedef typename dgboost::allocator_pointer<bucket_allocator_type>::type
          bucket_pointer;
        typedef dgboost::pointer_traits<bucket_pointer> bucket_pointer_traits;

        typedef bucket_group<Bucket> group;
        typedef typename dgboost::allocator_rebind<Allocator, group>::type
          group_allocator_type;
        typedef typename dgboost::allocator_pointer<group_allocator_type>::type
          group_pointer;
        typedef typename dgboost::pointer_traits<group_pointer>
      ...
      public:
              ...
              size_(size_policy::size(size_index_)), buckets(), groups()
        {
          ...
          buckets = dgboost::allocator_allocate(bucket_alloc, num_buckets);
          BOOST_TRY
          {
            groups = dgboost::allocator_allocate(group_alloc, num_groups);

            bucket_type* pb = dgboost::to_address(buckets);
            for (size_type i = 0; i < num_buckets; ++i) {
              new (pb + i) bucket_type();
            }

            group* pg = dgboost::to_address(groups);
          ...
          BOOST_CATCH(...)
          {
            dgboost::allocator_deallocate(bucket_alloc, buckets, num_buckets);
            BOOST_RETHROW
          }
          BOOST_CATCH_END
          ...
          BOOST_RV_REF(grouped_bucket_array) other) BOOST_NOEXCEPT
        {
          ...
          if (this == dgboost::addressof(other)) {
            return *this;
          }
        ...
        void swap(grouped_bucket_array& other)
        {
          ...
          bool b = dgboost::allocator_propagate_on_container_swap<
            allocator_type>::type::value;
          if (b) {
            dgboost::swap(get_node_allocator(), other.get_node_allocator());
          }
        }
        ...
        span<Bucket> raw()
        {
          ...
          return span<Bucket>(dgboost::to_address(buckets), size_);
        }
        ...
        void append_bucket_group(iterator itb) BOOST_NOEXCEPT
        {
          ...
          if (is_empty_bucket) {
            ...
            std::size_t n =
              static_cast<std::size_t>(dgboost::to_address(pb) - &buckets[0]);

...
#include <dgboost/unordered/detail/set.hpp>
...
#undef BOOST_UNORDERED_EMPLACE
  ...
  (BOOST_COMP_GNUC && BOOST_COMP_GNUC < BOOST_VERSION_NUMBER(4, 6, 0))
    ...
    public:
      ...
      void merge(dgboost::unordered_set<T, H2, P2, A>& source);
      ...
      template <typename H2, typename P2>
      void merge(dgboost::unordered_multiset<T, H2, P2, A>& source);
    ...
    template <class InputIterator,
      class Hash =
        dgboost::hash<typename std::iterator_traits<InputIterator>::value_type>,
      class Pred =
    ...
    template <class T, class Hash = dgboost::hash<T>,
    ...
    unordered_set(std::initializer_list<T>, std::size_t, Allocator)
      ->unordered_set<T, dgboost::hash<T>, std::equal_to<T>, Allocator>;
...
#undef BOOST_UNORDERED_EMPLACE
  ...
  (BOOST_COMP_GNUC && BOOST_COMP_GNUC < BOOST_VERSION_NUMBER(4, 6, 0))
    ...
    unordered_multiset(std::initializer_list<T>, std::size_t, Allocator)
      ->unordered_multiset<T, dgboost::hash<T>, std::equal_to<T>, Allocator>;
    ...
    void unordered_set<T, H, P, A>::merge(
      dgboost::unordered_set<T, H2, P2, A>& source)
    {
    ...
    template <typename H2, typename P2>
    void unordered_set<T, H, P, A>::merge(
      dgboost::unordered_multiset<T, H2, P2, A>& source)
    {
      table_.merge_unique(source.table_);
    }
    ...
    template <typename N, typename T, typename A> class node_handle_set
    {
      ...
      typedef typename dgboost::unordered::detail::rebind_wrap<A, T>::type
        value_allocator;

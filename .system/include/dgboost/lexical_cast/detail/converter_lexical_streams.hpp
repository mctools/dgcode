...
#       error "dgboost::lexical_cast to use only 'C' locale during conversions."
#   endif
#endif
...
#include <dgboost/lexical_cast/detail/lcast_char_constants.hpp>
#include <dgboost/lexical_cast/detail/lcast_unsigned_converters.hpp>
#include <dgboost/lexical_cast/detail/inf_nan.hpp>
...
#include <dgboost/detail/basic_pointerbuf.hpp>
...
namespace dgboost {} namespace boost = dgboost; namespace dgboost {
    ...
    namespace detail // optimized stream wrappers
    {
        template< class CharT // a result of widest_char transformation
                ...
                >
        class lexical_istream_limited_src: dgboost::noncopyable {
...
/************************************ HELPER FUNCTIONS FOR OPERATORS << ( ... ) ********************************/
            ...
            bool shl_char(T ch) {
                BOOST_STATIC_ASSERT_MSG(( sizeof(T) <= sizeof(CharT)) ,
                    "dgboost::lexical_cast does not support narrowing of char types."
                    "Use dgboost::locale instead" );
#ifndef BOOST_LEXICAL_CAST_ASSUME_C_LOCALE
                std::locale loc;
            ...
            bool shl_input_streamable(InputStreamable& input) {
                ...
                BOOST_STATIC_ASSERT((dgboost::is_same<char, CharT>::value));
#endif
            ...
            inline bool shl_signed(const T n) {
                CharT* tmp_finish = buffer + CharacterBufferSize;
                typedef BOOST_DEDUCED_TYPENAME dgboost::make_unsigned<T>::type utype;
            ...
            bool shl_real_type(float val, char* begin) {
                ...
                finish = start +
                    ...
                    sprintf(begin,
#endif
                    "%.*g", static_cast<int>(dgboost::detail::lcast_get_precision<float>()), val_as_double);
                return finish > start;
            }
            ...
            bool shl_real_type(double val, char* begin) {
                ...
                finish = start +
                    ...
                    sprintf(begin,
#endif
                    "%.*g", static_cast<int>(dgboost::detail::lcast_get_precision<double>()), val);
                return finish > start;
            }
            ...
            bool shl_real_type(long double val, char* begin) {
                ...
                finish = start +
                    ...
                    sprintf(begin,
#endif
                    "%.*Lg", static_cast<int>(dgboost::detail::lcast_get_precision<long double>()), val );
                return finish > start;
            }
#endif
            ...
            bool shl_real_type(float val, wchar_t* begin) {
                ...
                finish = start + swprintf(begin, CharacterBufferSize,
                                       L"%.*g",
                                       static_cast<int>(dgboost::detail::lcast_get_precision<float >()),
                                       val_as_double );
                return finish > start;
            }
            ...
            bool shl_real_type(double val, wchar_t* begin) {
                using namespace std;
                finish = start + swprintf(begin, CharacterBufferSize,
                                          L"%.*g", static_cast<int>(dgboost::detail::lcast_get_precision<double >()), val );
                return finish > start;
            }
            ...
            bool shl_real_type(long double val, wchar_t* begin) {
                using namespace std;
                finish = start + swprintf(begin, CharacterBufferSize,
                                          L"%.*Lg", static_cast<int>(dgboost::detail::lcast_get_precision<long double >()), val );
                return finish > start;
            }
#endif
            template <class T>
...
/************************************ OPERATORS << ( ... ) ********************************/
        public:
            ...
            template<class Alloc>
            bool operator<<(dgboost::container::basic_string<CharT,Traits,Alloc> const& str) BOOST_NOEXCEPT {
                start = str.data();
                finish = start + str.length();
                return true;
            }
            ...
            template <class C>
            BOOST_DEDUCED_TYPENAME dgboost::disable_if<dgboost::is_const<C>, bool>::type
            ...
            bool operator<<(dgboost::ulong_long_type n)   { return shl_unsigned(n); }
            bool operator<<(dgboost::long_long_type n)    { return shl_signed(n); }
            ...
            bool operator<<(const dgboost::uint128_type& n)   { return shl_unsigned(n); }
            bool operator<<(const dgboost::int128_type& n)    { return shl_signed(n); }
#endif
            ...
            BOOST_DEDUCED_TYPENAME dgboost::disable_if<dgboost::is_const<C>, bool>::type
            operator<<(dgboost::array<C, N> const& input) BOOST_NOEXCEPT {
                BOOST_STATIC_ASSERT_MSG(
                    (sizeof(dgboost::array<const C, N>) == sizeof(dgboost::array<C, N>)),
                    "dgboost::array<C, N> and dgboost::array<const C, N> must have exactly the same layout."
                );
                return ((*this) << reinterpret_cast<dgboost::array<const C, N> const& >(input));
            }

            template <std::size_t N>
            bool operator<<(dgboost::array<const CharT, N> const& input) BOOST_NOEXCEPT {
                return shl_char_array_limited(input.data(), N);
            }

            template <std::size_t N>
            bool operator<<(dgboost::array<const unsigned char, N> const& input) BOOST_NOEXCEPT {
                return ((*this) << reinterpret_cast<dgboost::array<const char, N> const& >(input));
            }

            template <std::size_t N>
            bool operator<<(dgboost::array<const signed char, N> const& input) BOOST_NOEXCEPT {
                return ((*this) << reinterpret_cast<dgboost::array<const char, N> const& >(input));
            }
            ...
            bool operator<<(std::array<C, N> const& input) BOOST_NOEXCEPT {
                BOOST_STATIC_ASSERT_MSG(
                    (sizeof(std::array<C, N>) == sizeof(dgboost::array<C, N>)),
                    "std::array and dgboost::array must have exactly the same layout. "
                    "Bug in implementation of std::array or dgboost::array."
                );
                return ((*this) << reinterpret_cast<dgboost::array<C, N> const& >(input));
            }
#endif
        ...
        template <class CharT, class Traits>
        class lexical_ostream_limited_src: dgboost::noncopyable {
...
/************************************ HELPER FUNCTIONS FOR OPERATORS >> ( ... ) ********************************/
        private:
            ...
            {
                BOOST_STATIC_ASSERT_MSG(
                    (!dgboost::is_pointer<InputStreamable>::value),
                    "dgboost::lexical_cast can not convert to pointers"
                );

#if defined(BOOST_NO_STRINGSTREAM) || defined(BOOST_NO_STD_LOCALE)
                BOOST_STATIC_ASSERT_MSG((dgboost::is_same<char, CharT>::value),
                    "dgboost::lexical_cast can not convert, because your STL library does not "
            ...
            inline bool shr_xchar(T& output) BOOST_NOEXCEPT {
                BOOST_STATIC_ASSERT_MSG(( sizeof(CharT) == sizeof(T) ),
                    "dgboost::lexical_cast does not support narrowing of character types."
                    "Use dgboost::locale instead" );
...
/************************************ OPERATORS >> ( ... ) ********************************/
        public:
...
#if defined(BOOST_HAS_LONG_LONG)
            bool operator>>(dgboost::ulong_long_type& output)     { return shr_unsigned(output); }
            bool operator>>(dgboost::long_long_type& output)      { return shr_signed(output); }
#elif defined(BOOST_HAS_MS_INT64)
...
#ifdef BOOST_HAS_INT128
            bool operator>>(dgboost::uint128_type& output)        { return shr_unsigned(output); }
            bool operator>>(dgboost::int128_type& output)         { return shr_signed(output); }
#endif
            ...
            template<class Alloc>
            bool operator>>(dgboost::container::basic_string<CharT,Traits,Alloc>& str) {
                str.assign(start, finish); return true;
            }

            template <std::size_t N>
            bool operator>>(dgboost::array<CharT, N>& output) BOOST_NOEXCEPT {
                return shr_std_array<N>(output);
            }

            template <std::size_t N>
            bool operator>>(dgboost::array<unsigned char, N>& output) BOOST_NOEXCEPT {
                return ((*this) >> reinterpret_cast<dgboost::array<char, N>& >(output));
            }

            template <std::size_t N>
            bool operator>>(dgboost::array<signed char, N>& output) BOOST_NOEXCEPT {
                return ((*this) >> reinterpret_cast<dgboost::array<char, N>& >(output));
            }
            ...
            bool operator>>(std::array<C, N>& output) BOOST_NOEXCEPT {
                BOOST_STATIC_ASSERT_MSG(
                    (sizeof(std::array<C, N>) == sizeof(dgboost::array<C, N>)),
                    "std::array<C, N> and dgboost::array<C, N> must have exactly the same layout."
                );
                return ((*this) >> reinterpret_cast<dgboost::array<C, N>& >(output));
            }
#endif

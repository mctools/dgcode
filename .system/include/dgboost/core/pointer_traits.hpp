...
-> decltype(dgboost::pointer_traits<T>::to_address(v))
{
    return dgboost::pointer_traits<T>::to_address(v);
}
...
{
    return dgboost::detail::ptr_address(v.operator->(), 0);
}
...
{
    return dgboost::detail::ptr_address(v, 0);
}
#else
template<class T>
...
to_address(const T& v) BOOST_NOEXCEPT
{
    return dgboost::to_address(v.operator->());
}
#endif

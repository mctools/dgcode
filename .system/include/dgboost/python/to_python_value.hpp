...
#include <dgboost/python/converter/builtin_converters.hpp>
#include <dgboost/python/converter/object_manager.hpp>
#include <dgboost/python/converter/shared_ptr_to_python.hpp>

#include <dgboost/python/detail/type_traits.hpp>
#include <dgboost/python/detail/value_is_shared_ptr.hpp>
...
struct object_manager_get_pytype<true>
{
...
#ifndef BOOST_PYTHON_NO_PY_SIGNATURES
      typedef dgboost::mpl::bool_<is_handle<T>::value> is_t_handle;
      typedef dgboost::detail::indirect_traits::is_reference_to_const<T> is_t_const;
  ...
  struct shared_ptr_to_python_value
  {
      ...
      PyObject* operator()(argument_type) const;
#ifndef BOOST_PYTHON_NO_PY_SIGNATURES
      PyTypeObject const* get_pytype() const {return get_pytype((dgboost::type<argument_type>*)0);}
#endif 
      // This information helps make_getter() decide whether to try to
      // return an internal reference or not. I don't like it much,
      // but it will have to serve for now.
...
#ifndef BOOST_PYTHON_NO_PY_SIGNATURES
    template <class U>
    PyTypeObject const* get_pytype(dgboost::type<shared_ptr<U> &> *) const {return converter::registered<U>::converters.to_python_target_type();}
    template <class U>
    PyTypeObject const* get_pytype(dgboost::type<const shared_ptr<U> &> *) const {return converter::registered<U>::converters.to_python_target_type();}
# if !defined(BOOST_NO_CXX11_SMART_PTR)
    template <class U>
    PyTypeObject const* get_pytype(dgboost::type<std::shared_ptr<U> &> *) const {return converter::registered<U>::converters.to_python_target_type();}
    template <class U>
    PyTypeObject const* get_pytype(dgboost::type<const std::shared_ptr<U> &> *) const {return converter::registered<U>::converters.to_python_target_type();}
# endif
#endif
  };
}

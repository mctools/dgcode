...
# include <dgboost/python/call.hpp>
...
# include <dgboost/python/def_visitor.hpp>

# include <dgboost/python/detail/raw_pyobject.hpp>
# include <dgboost/python/detail/dependent.hpp>

# include <dgboost/python/object/forward.hpp>
# include <dgboost/python/object/add_to_namespace.hpp>
...
# include <dgboost/python/detail/string_literal.hpp>
# include <dgboost/python/detail/def_helper_fwd.hpp>
...
// attribute/item access or addition.
  ...
  //
  BOOST_PYTHON_IS_XXX_DEF(proxy, dgboost::python::api::proxy, 1)
...
# define BOOST_PP_ITERATION_PARAMS_1 (3, (1, BOOST_PYTHON_MAX_ARITY, <dgboost/python/object_call.hpp>))
...
# endif
  ...
  struct is_derived
    : dgboost::python::detail::is_convertible<
...
# define BOOST_PYTHON_FORWARD_OBJECT_CONSTRUCTORS(derived, base)               \
    inline explicit derived(::dgboost::python::detail::borrowed_reference p)     \
        : base(p) {}                                                           \
    inline explicit derived(::dgboost::python::detail::new_reference p)          \
        : base(p) {}                                                           \
    inline explicit derived(::dgboost::python::detail::new_non_null_reference p) \
        : base(p) {}
  ...
  struct object_initializer_impl<false, true>
  {
      ...
      get(T const& x, U)
      {
          return python::incref(get_managed_object(x, dgboost::python::tag));
      }
  };
...
{ 
  ...
  return dgboost::python::detail::args_proxy(x); 
} 
...
{ 
  ...
  PyObject *result = PyObject_Call(get_managed_object(self, dgboost::python::tag), 
                                   args.operator object().ptr(), 
                                   0); 
  return object(dgboost::python::detail::new_reference(result)); 
...
# include <dgboost/python/slice_nil.hpp>

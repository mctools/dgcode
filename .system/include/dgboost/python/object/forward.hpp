...
// Very much like dgboost::reference_wrapper<T>, except that in this
// case T can be a reference already without causing a
// reference-to-reference error.
template <class T>
struct reference_to_value
{
    typedef typename dgboost::python::detail::add_lvalue_reference<typename
        dgboost::python::detail::add_const<T>::type>::type reference;
...
struct forward
    : mpl::if_<
          mpl::or_<python::detail::copy_ctor_mutates_rhs<T>, dgboost::python::detail::is_scalar<T> >
        , T
...
struct unforward_cref<reference_to_value<T> >
  : dgboost::python::detail::add_lvalue_reference<typename dgboost::python::detail::add_const<T>::type>
{
};

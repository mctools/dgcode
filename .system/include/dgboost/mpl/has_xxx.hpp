...
#include <dgboost/mpl/aux_/config/has_xxx.hpp>
#include <dgboost/mpl/aux_/config/msvc_typename.hpp>
...
#include <dgboost/preprocessor/array/elem.hpp>
...
template< typename T, typename name = ::dgboost::mpl::aux::has_xxx_tag > \
struct BOOST_PP_CAT(trait,_impl) : T \
{ \
    static dgboost::mpl::aux::no_tag \
    test(void(*)(::dgboost::mpl::aux::has_xxx_tag)); \
    \
    static dgboost::mpl::aux::yes_tag test(...); \
    \
    BOOST_STATIC_CONSTANT(bool, value = \
          sizeof(test(static_cast<void(*)(name)>(0))) \
            != sizeof(dgboost::mpl::aux::no_tag) \
        ); \
    typedef dgboost::mpl::bool_<value> type; \
}; \
\
template< typename T, typename fallback_ = dgboost::mpl::bool_<default_> > \
struct trait \
    : dgboost::mpl::if_c< \
          dgboost::mpl::aux::msvc_is_incomplete<T>::value \
        , dgboost::mpl::bool_<false> \
...
{ \
    BOOST_STATIC_CONSTANT(bool, value = false); \
    typedef dgboost::mpl::bool_<false> type; \
}; \
/**/
...
{ \
    ...
    { \
        template < class U > \
        static int  resolve( dgboost::mpl::aux::type_wrapper<U> const volatile * \
                           , dgboost::mpl::aux::type_wrapper<typename U::name >* = 0 ); \
        static char resolve( ... ); \
    }; \
    typedef dgboost::mpl::aux::type_wrapper<T> t_; \
    BOOST_STATIC_CONSTANT( bool, value = ( sizeof( trait_tester_impl::resolve( static_cast< t_ * >(0) ) ) == sizeof(int) ) ); \
}; \
template< typename T, typename fallback_ = dgboost::mpl::bool_<default_> > \
struct trait           \
{                      \
    BOOST_STATIC_CONSTANT( bool, value = (trait_tester< T, dgboost::is_class< T >::value >::value) );     \
    typedef dgboost::mpl::bool_< trait< T, fallback_ >::value > type; \
};
...
{ \
    ...
    { \
        template< typename U > \
        static dgboost::mpl::aux::yes_tag test( \
              dgboost::mpl::aux::type_wrapper<U> const volatile* \
            , dgboost::mpl::aux::type_wrapper<BOOST_MSVC_TYPENAME U::name>* = 0 \
            ); \
    \
        static dgboost::mpl::aux::no_tag test(...); \
    }; \
    \
    typedef dgboost::mpl::aux::type_wrapper<T> t_; \
    BOOST_STATIC_CONSTANT(bool, value = \
          sizeof(gcc_3_2_wknd::test(static_cast<t_*>(0))) \
            == sizeof(dgboost::mpl::aux::yes_tag) \
        ); \
    typedef dgboost::mpl::bool_<value> type; \
}; \
/**/
...
#     define BOOST_MPL_HAS_MEMBER_MULTI_ACCEPT(z, n, args) \
        template< typename V > \
        static dgboost::mpl::aux::yes_tag \
        BOOST_MPL_HAS_MEMBER_INTROSPECTION_TEST_NAME(args)( \
            dgboost::mpl::aux::type_wrapper< V > const volatile* \
...
#       define BOOST_MPL_HAS_MEMBER_TEST(args) \
          sizeof( \
              BOOST_MPL_HAS_MEMBER_INTROSPECTION_TEST_NAME(args)( \
                  static_cast< dgboost::mpl::aux::type_wrapper< U >* >(0) \
              ) \
          ) == sizeof(dgboost::mpl::aux::yes_tag) \
        /**/
#     else
...
#   define BOOST_MPL_HAS_MEMBER_IMPLEMENTATION( \
      ...
      template< \
          typename T \
        , typename fallback_ \
              = dgboost::mpl::bool_< BOOST_PP_ARRAY_ELEM(3, args) > \
      > \
...
#   define BOOST_MPL_HAS_XXX_TEMPLATE_NAMED_DEF(trait, name, default_) \
      template< typename T \
              , typename fallback_ = dgboost::mpl::bool_< default_ > > \
      struct trait { \
          BOOST_STATIC_CONSTANT(bool, value = fallback_::value); \
          typedef fallback_ type; \
      }; \

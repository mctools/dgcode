...
#include <dgboost/serialization/extended_type_info.hpp>
#include <dgboost/serialization/factory.hpp>
#include <dgboost/serialization/throw_exception.hpp>

#include <dgboost/serialization/config.hpp>
// hijack serialization access
#include <dgboost/serialization/access.hpp>

#include <dgboost/config/abi_prefix.hpp> // must be the last header
#ifdef BOOST_MSVC
#  pragma warning(push)
...
public:
    ...
    is_less_than(const dgboost::serialization::extended_type_info &rhs) const ;
    virtual BOOST_SERIALIZATION_DECL bool
    is_equal(const dgboost::serialization::extended_type_info &rhs) const ;
};
...
public:
    ...
    get_derived_extended_type_info(const T & t) const {
        ...
        BOOST_STATIC_WARNING(dgboost::is_polymorphic< T >::value);
        const char * derived_key = t.get_key();
        BOOST_ASSERT(NULL != derived_key);
        return dgboost::serialization::extended_type_info::find(derived_key);
    }
    ...
    virtual void * construct(unsigned int count, ...) const{
        ...
        switch(count){
        case 0:
            return factory<typename dgboost::remove_const< T >::type, 0>(ap);
        case 1:
            return factory<typename dgboost::remove_const< T >::type, 1>(ap);
        case 2:
            return factory<typename dgboost::remove_const< T >::type, 2>(ap);
        case 3:
            return factory<typename dgboost::remove_const< T >::type, 3>(ap);
        case 4:
            return factory<typename dgboost::remove_const< T >::type, 4>(ap);
        default:
    ...
    virtual void destroy(void const * const p) const{
        dgboost::serialization::access::destroy(
...
// before any of the other headers.
    ...
    struct extended_type_info_impl {
        typedef typename 
            dgboost::serialization::extended_type_info_no_rtti< T > type;
    };
    } // namespace serialization
    } // namespace dgboost
#endif
...
#  pragma warning(pop)
#endif
#include <dgboost/config/abi_suffix.hpp> // pops abi_suffix.hpp pragmas

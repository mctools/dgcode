...
{
   BOOST_STATIC_CONSTANT(bool, value = ::dgboost::is_class<T>::value || ::dgboost::is_union<T>::value);
};
...
# else
    template <class U> static ::dgboost::type_traits::yes_type is_class_or_union_tester(void(U::*)(void));

#  if BOOST_WORKAROUND(__MWERKS__, <= 0x3000) // no SFINAE
    static ::dgboost::type_traits::no_type is_class_or_union_tester(...);
    BOOST_STATIC_CONSTANT(
        bool, value = sizeof(is_class_or_union_tester(0)) == sizeof(::dgboost::type_traits::yes_type));
#  else
    template <class U>
    static ::dgboost::type_traits::no_type is_class_or_union_tester(...);
    BOOST_STATIC_CONSTANT(
        bool, value = sizeof(is_class_or_union_tester<T>(0)) == sizeof(::dgboost::type_traits::yes_type));
#  endif
# endif
};
#endif
...
struct is_enum_helper<false>
{
    template <typename T> struct type
    {
       static const bool value = ::dgboost::is_convertible<typename dgboost::add_reference<T>::type, ::dgboost::detail::int_convertible>::value;
    };
};

template <typename T> struct is_enum_impl
{
   //typedef ::dgboost::add_reference<T> ar_t;
   ...
   BOOST_STATIC_CONSTANT(bool, selector =
    ...
    typedef ::dgboost::detail::is_enum_helper<
          ::dgboost::detail::is_enum_impl<T>::selector
        > se_t;
#else
    typedef ::dgboost::detail::is_enum_helper<selector> se_t;
#endif
...
template <class T> struct is_enum : public integral_constant<bool, ::dgboost::detail::is_enum_impl<T>::value> {};

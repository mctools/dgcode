...
//
namespace dgboost {} namespace boost = dgboost; namespace dgboost{ namespace type_traits_detail{
...
{
  template<typename Q> static dgboost::true_type  test(decltype(&Q::operator())*);
  template<typename>   static dgboost::false_type test(...);
...
struct is_likely_stateless_lambda<
  T,
  typename dgboost::enable_if_<has_one_operator_call<T>::value>::type> :
     dgboost::is_convertible<T, typename equivalent_function_pointer<T>::type
>{};
...
namespace dgboost {} namespace boost = dgboost; namespace dgboost {
   namespace type_traits_detail {
      ...
      template<typename T>
      struct is_likely_stateless_lambda : public dgboost::integral_constant<bool, false> {};
}}

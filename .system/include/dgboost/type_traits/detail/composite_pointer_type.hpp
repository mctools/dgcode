...
private:
    ...
    typedef typename dgboost::remove_cv<T>::type T2;
    typedef typename dgboost::remove_cv<U>::type U2;

public:

    BOOST_STATIC_CONSTANT( bool, value =
        (dgboost::is_same<T2, U2>::value)
        || dgboost::is_void<T2>::value
        || dgboost::is_void<U2>::value
        || (dgboost::is_base_of<T2, U2>::value)
        || (dgboost::is_base_of<U2, T2>::value) );
};
...
public:
    ...
    typedef typename dgboost::conditional<

        dgboost::is_same<T2, U2>::value || dgboost::is_void<T2>::value || dgboost::is_base_of<T2, U2>::value,
        typename dgboost::copy_cv<T, U>::type,
        typename dgboost::copy_cv<U, T>::type
...
public:
    ...
    typedef typename dgboost::copy_cv<typename dgboost::copy_cv<typename composite_pointer_type<T2, U2>::type const, T>::type, U>::type type;
};

//Old compilers like MSVC-7.1 have problems using dgboost::conditional in

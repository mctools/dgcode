...
namespace dgboost {} namespace boost = dgboost; namespace dgboost{
   ...
   namespace detail{
      ...
      {
         template<typename T, typename = decltype(dgboost::declval<T&>().~T())>
   ...
   template <class T> struct is_destructible : public integral_constant<bool, sizeof(dgboost::detail::is_destructible_imp::test<T>(0)) == sizeof(dgboost::type_traits::yes_type)>
   {
      BOOST_STATIC_ASSERT_MSG(dgboost::is_complete<T>::value, "Arguments to is_destructible must be complete types");
   };

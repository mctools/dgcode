...
template <class T> struct is_copy_constructible : public dgboost::is_constructible<T, const T&>{};
...
// dgboost::non_copyable as a base class as a special case:
//
...
namespace dgboost {} namespace boost = dgboost; namespace dgboost {
   ...
   namespace detail
   {
      ...
      template <class T, bool b> struct is_copy_constructible_imp : public dgboost::is_constructible<T, const T&>{};
...
namespace dgboost {} namespace boost = dgboost; namespace dgboost {
   ...
   namespace detail{
      ...
      struct is_copy_constructible_impl2 {
         ...
         template <class T1>
         static dgboost::type_traits::yes_type test(const T1&, dgboost::mpl::int_<sizeof(T1(dgboost::declval<const T1&>()))>* = 0);
#else
         template <class T1>
         static dgboost::type_traits::yes_type test(const T1&, decltype(T1(dgboost::declval<const T1&>()))* = 0);
#endif

         static dgboost::type_traits::no_type test(...);
#else
         template <class T1>
         static dgboost::type_traits::no_type test(const T1&, typename T1::boost_move_no_copy_constructor_or_assign* = 0);
         ...
         BOOST_STATIC_CONSTANT(bool, value = (
            sizeof(test(
            dgboost::declval<BOOST_DEDUCED_TYPENAME dgboost::add_reference<T const>::type>()
            )) == sizeof(dgboost::type_traits::yes_type)
            &&
            !dgboost::is_rvalue_reference<T>::value
      ...
      struct is_copy_constructible_impl {
         ...
         BOOST_STATIC_CONSTANT(bool, value = (
            dgboost::detail::is_copy_constructible_impl2<
   ...
   template <class T> struct is_copy_constructible : public integral_constant<bool, ::dgboost::detail::is_copy_constructible_impl<T>::value>{};
   template <> struct is_copy_constructible<void> : public false_type{};

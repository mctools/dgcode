...
#include <dgboost/operators.hpp>
#include <dgboost/property_map/property_map.hpp>
#include <dgboost/pending/container_traits.hpp>
#include <dgboost/range/irange.hpp>
#include <dgboost/graph/graph_traits.hpp>
#include <memory>
#include <iterator>
#include <algorithm>
#include <dgboost/limits.hpp>

#include <dgboost/iterator/iterator_adaptor.hpp>

#include <dgboost/mpl/if.hpp>
#include <dgboost/mpl/not.hpp>
#include <dgboost/mpl/and.hpp>
#include <dgboost/graph/graph_concepts.hpp>
#include <dgboost/pending/container_traits.hpp>
#include <dgboost/graph/detail/adj_list_edge_iterator.hpp>
#include <dgboost/graph/properties.hpp>
#include <dgboost/pending/property.hpp>
#include <dgboost/graph/adjacency_iterator.hpp>
...
namespace detail
{
    ...
    class stored_edge
    : public dgboost::equality_comparable1< stored_edge< Vertex >,
          dgboost::less_than_comparable1< stored_edge< Vertex > > >
    {
    public:
...
namespace detail
{
    ...
    inline void remove_directed_edge_if_dispatch(incidence_iterator first,
        ...
        dgboost::allow_parallel_edge_tag)
    {
        // remove_if
    ...
    inline void remove_directed_edge_if_dispatch(incidence_iterator first,
        incidence_iterator last, EdgeList& el, Predicate pred,
        dgboost::disallow_parallel_edge_tag)
    {
    ...
    inline void undirected_remove_out_edge_if_dispatch(Graph& g,
        ...
        Predicate pred, dgboost::allow_parallel_edge_tag)
    {
    ...
    inline void undirected_remove_out_edge_if_dispatch(Graph& g,
        incidence_iterator first, incidence_iterator last, EdgeList& el,
        Predicate pred, dgboost::disallow_parallel_edge_tag)
    {
        typedef typename Graph::global_edgelist_selector EdgeListS;
        BOOST_STATIC_ASSERT((!is_same< EdgeListS, vecS >::value));
    ...
    Predicate pred, directed_graph_helper< Config >& g_)
{
    ...
    dgboost::tie(first, last) = out_edges(u, g);
...
inline void remove_edge_if(Predicate pred, directed_graph_helper< Config >& g_)
{
    ...
    for (dgboost::tie(vi, vi_end) = vertices(g); vi != vi_end; ++vi)
    ...
    typename Config::vertex_descriptor u, directed_graph_helper< Config >& g_)
{
    ...
    for (dgboost::tie(vi, viend) = vertices(g); vi != viend; ++vi)
    ...
    directed_graph_helper< Config >& g_)
{
    ...
    bool inserted;
    dgboost::tie(i, inserted)
        = dgboost::graph_detail::push(g.out_edge_list(u), StoredEdge(v, p));
...
namespace detail
{
    ...
    inline void remove_edge_and_property(
        Graph& g, EdgeList& el, Vertex v, dgboost::allow_parallel_edge_tag cat)
    {
        typedef typename Graph::global_edgelist_selector EdgeListS;
        BOOST_STATIC_ASSERT((!is_same< EdgeListS, vecS >::value));
        ...
        for (; i != end; ++i)
        {
            if ((*i).get_target() == v)
            {
                ...
                // for multisets also.
                bool skip = (dgboost::next(i) != end
                    && i->get_iter() == dgboost::next(i)->get_iter());
    ...
    template < class Graph, class EdgeList, class Vertex >
    inline void remove_edge_and_property(
        Graph& g, EdgeList& el, Vertex v, dgboost::disallow_parallel_edge_tag)
    {
        typedef typename Graph::global_edgelist_selector EdgeListS;
        BOOST_STATIC_ASSERT((!is_same< EdgeListS, vecS >::value));
...
: public dgboost::detail::edge_base< dgboost::undirected_tag, Vertex >
{
    typedef EdgeProperty property_type;
    typedef dgboost::detail::edge_base< dgboost::undirected_tag, Vertex > Base;
    ...
    typename Config::vertex_descriptor u, undirected_graph_helper< Config >& g_)
{
    ...
    while (true)
    {
        ...
        dgboost::tie(ei, ei_end) = out_edges(u, g);
        if (ei == ei_end)
            break;
        remove_edge(*ei, g);
    }
    ...
    undirected_graph_helper< Config >& g_)
{
    ...
    typename Config::OutEdgeList::iterator i;
    dgboost::tie(i, inserted) = dgboost::graph_detail::push(
        g.out_edge_list(u), StoredEdge(v, p_iter, &g.m_edges));
    if (inserted)
    {
        dgboost::graph_detail::push(
    ...
    Predicate pred, bidirectional_graph_helper_with_property< Config >& g_)
{
    ...
    for (dgboost::tie(out_i, out_end) = out_edges(u, g); out_i != out_end;
         ++out_i)
        if (pred(*out_i))
        {
    ...
    Predicate pred, bidirectional_graph_helper_with_property< Config >& g_)
{
    ...
    for (dgboost::tie(in_i, in_end) = in_edges(v, g); in_i != in_end; ++in_i)
        if (pred(*in_i))
        {
    ...
    dgboost::tie(first, last) = in_edges(v, g);
    typedef typename Config::edge_parallel_category Cat;
    detail::remove_directed_edge_if_dispatch(
    ...
    const adj_list_helper< Config, Base >& g_)
{
    ...
    typedef typename Config::inv_adjacency_iterator inv_adjacency_iterator;
    typename Config::in_edge_iterator first, last;
    dgboost::tie(first, last) = in_edges(u, g);
    return std::make_pair(
    ...
    const adj_list_helper< Config, Base >& g_)
{
    ...
    dgboost::tie(first, last) = graph_detail::equal_range(el, StoredEdge(v));
    return std::make_pair(
...
namespace detail
{
    ...
    inline typename dgboost::property_map< typename Config::graph_type,
        Property >::type
    get_dispatch(
        adj_list_helper< Config, Base >&, Property p, dgboost::edge_property_tag)
    {
        typedef typename Config::graph_type Graph;
        typedef typename dgboost::property_map< Graph, Property >::type PA;
        return PA(p);
    }
    template < class Config, class Base, class Property >
    inline typename dgboost::property_map< typename Config::graph_type,
    ...
    get_dispatch(const adj_list_helper< Config, Base >&, Property p,
        dgboost::edge_property_tag)
    {
        typedef typename Config::graph_type Graph;
        typedef typename dgboost::property_map< Graph, Property >::const_type PA;
        return PA(p);
    }
...
template < class Config, class Base, class Property >
inline
    typename dgboost::property_map< typename Config::graph_type, Property >::type
...
inline typename dgboost::property_traits< typename dgboost::property_map<
...
{
    typedef typename Config::graph_type Graph;
    typedef typename dgboost::property_map< Graph, Property >::type Map;
...
public:
    ...
    inline void copy_impl(const adj_list_impl& x_)
    {
        ...
        for (dgboost::tie(vi, vi_end) = vertices(x); vi != vi_end; ++vi)
        {
        ...
        for (dgboost::tie(ei, ei_end) = edges(x); ei != ei_end; ++ei)
        {
            edge_descriptor e;
            bool inserted;
            vertex_descriptor s = source(*ei, x), t = target(*ei, x);
            dgboost::tie(e, inserted) = add_edge(vertex_map[(stored_vertex*)s],
    ...
    adj_list_impl< Derived, Config, Base >& g_)
{
    ...
    bool inserted;
    dgboost::tie(pos, inserted) = dgboost::graph_detail::push(g.m_vertices, v);
    v->m_position = pos;
    g.added_vertex(v);
    return v;
}
    ...
    adj_list_impl< Derived, Config, Base >& g_)
{
    ...
    Derived& g = static_cast< Derived& >(g_);
    g.removing_vertex(
        u, dgboost::graph_detail::iterator_stability(g_.m_vertices));
...
namespace detail
{
    ...
    inline void remove_vertex_dispatch(
        Graph& g, vertex_descriptor u, dgboost::directed_tag)
    {
    ...
    template < class Graph, class vertex_descriptor >
    inline void remove_vertex_dispatch(
        Graph& g, vertex_descriptor u, dgboost::undirected_tag)
    {
        typedef typename Graph::global_edgelist_selector EdgeListS;
        BOOST_STATIC_ASSERT((!is_same< EdgeListS, vecS >::value));
    ...
    template < class Graph, class vertex_descriptor >
    inline void remove_vertex_dispatch(
        Graph& g, vertex_descriptor u, dgboost::bidirectional_tag)
    {
        typedef typename Graph::global_edgelist_selector EdgeListS;
        BOOST_STATIC_ASSERT((!is_same< EdgeListS, vecS >::value));
    ...
    inline void reindex_edge_list(
        EdgeList& el, vertex_descriptor u, dgboost::allow_parallel_edge_tag)
    {
    ...
    template < class EdgeList, class vertex_descriptor >
    inline void reindex_edge_list(
        EdgeList& el, vertex_descriptor u, dgboost::disallow_parallel_edge_tag)
    {
        typename EdgeList::iterator ei = el.begin(), e_end = el.end();
        while (ei != e_end)
        {
...
public:
    ...
    //    protected:
    inline dgboost::integer_range< vertex_descriptor > vertex_set() const
    {
        return dgboost::integer_range< vertex_descriptor >(0, m_vertices.size());
    }
    inline OutEdgeList& out_edge_list(vertex_descriptor v)
    {
    ...
    vec_adj_list_impl< Graph, Config, Base >& g_)
{
    ...
    Graph& g = static_cast< Graph& >(g_);
    g.removing_vertex(
        v, dgboost::graph_detail::iterator_stability(g_.m_vertices));
    ...
    struct adj_list_gen
    {
        ...
        struct config
        {
            ...
            typedef typename container_gen< VertexListS, vertex_ptr >::type
                SeqVertexList;
            typedef dgboost::integer_range< vertex_descriptor > RandVertexList;
...
: public dgboost::put_get_helper< Reference,
...
: public dgboost::put_get_helper< PropRef,
          ...
          Tag > >
{
    typedef ValueType value_type;
    typedef Reference reference;
    typedef typename dgboost::graph_traits< Graph >::vertex_descriptor key_type;
    typedef dgboost::lvalue_property_map_tag category;
...
: public dgboost::put_get_helper< PropertyRef,
...
: public dgboost::put_get_helper< Vertex,
      vec_adj_list_vertex_id_map< Property, Vertex > >
{
    typedef Vertex value_type;
    typedef Vertex key_type;
    typedef Vertex reference;
    typedef dgboost::readable_property_map_tag category;
...
namespace detail
{
    ...
    : dgboost::mpl::if_< dgboost::is_same< Tag, vertex_all_t >,
...
template < typename V > struct hash< dgboost::detail::stored_edge< V > >
{
    std::size_t operator()(const dgboost::detail::stored_edge< V >& e) const
    {
...
struct hash< dgboost::detail::stored_edge_property< V, P > >
{
    std::size_t operator()(
        const dgboost::detail::stored_edge_property< V, P >& e) const
    {
        return hash< V >()(e.m_target);
    }
};
...
struct hash< dgboost::detail::stored_edge_iter< V, I, P > >
{
    std::size_t operator()(
        const dgboost::detail::stored_edge_iter< V, I, P >& e) const
    {
        return hash< V >()(e.m_target);
    }
};

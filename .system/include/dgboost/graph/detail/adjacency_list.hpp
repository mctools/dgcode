...
namespace dgboost {} namespace boost = dgboost; namespace dgboost {
  ...
  namespace detail {
    ...
    {
      dgboost::graph_detail::erase_if(el, detail::target_is<vertex_descriptor>(v));
    }
    // O(log(E/V))
    template <class EdgeList, class vertex_descriptor>
    void erase_from_incidence_list(EdgeList& el, vertex_descriptor v,
      ...
      const detail::stored_ra_edge_iter<Vertex, EdgeVec, Property>& e)
  {
             ...
             directed_graph_helper<Config>& g_)
    {
      ...
      bool inserted;
      dgboost::tie(i, inserted) = dgboost::graph_detail::push(g.out_edge_list(u),
                                            StoredEdge(v, p));
    ...
    namespace detail {
      ...
      remove_edge_and_property(Graph& g, EdgeList& el, Vertex v,
                               dgboost::allow_parallel_edge_tag cat)
      {
        typedef typename Graph::global_edgelist_selector EdgeListS;
        BOOST_STATIC_ASSERT((!is_same<EdgeListS, vecS>::value));
        ...
        for (; i != end; ++i) {
          if ((*i).get_target() == v) {
            ...
            bool skip = (dgboost::next(i) != end && i->get_iter() == dgboost::next(i)->get_iter());
             ...
             undirected_graph_helper<Config>& g_)
    {
      ...
      if (inserted) {
        dgboost::graph_detail::push(g.out_edge_list(v), StoredEdge(u, p_iter, &g.m_edges));
                       ...
                       bidirectional_graph_helper_with_property<Config>& g_)
    {
      ...
      for (dgboost::tie(out_i, out_end) = out_edges(u, g); out_i != out_end; ++out_i)
        if (pred(*out_i)) {
          detail::remove_directed_edge_dispatch
             ...
             bidirectional_graph_helper_with_property<Config>& g_)
    {
      ...
      dgboost::tie(i, inserted) = dgboost::graph_detail::push(g.out_edge_list(u),
                                        StoredEdge(v, p_iter, &g.m_edges));
      if (inserted) {
        dgboost::graph_detail::push(in_edge_list(g, v), StoredEdge(u, p_iter, &g.m_edges));
               ...
               const adj_list_helper<Config, Base>& g_)
    {
      ...
      dgboost::tie(first, last) = graph_detail::
    ...
    namespace detail {
      ...
      inline
      typename dgboost::property_map<typename Config::graph_type,
        Property>::type
                              ...
                              adj_list_impl<Derived, Config, Base>& g_)
    {
      typedef typename Config::stored_vertex stored_vertex;
      Derived& g = static_cast<Derived&>(g_);
      g.removing_vertex(u, dgboost::graph_detail::iterator_stability(g_.m_vertices));
    ...
    public:
      ...
      inline void copy_impl(const vec_adj_list_impl& x_)
      {
        ...
        for (dgboost::tie(ei, ei_end) = edges(x); ei != ei_end; ++ei) {
          edge_descriptor e;
          bool inserted;
          dgboost::tie(e, inserted) = add_edge(source(*ei,x), target(*ei,x) , *this);
          *((edge_property_type*)e.m_eproperty)
            = *((edge_property_type*)(*ei).m_eproperty);
        }
      }
                              ...
                              vec_adj_list_impl<Graph, Config, Base>& g_)
    {
      ...
      Graph& g = static_cast<Graph&>(g_);
      g.removing_vertex(v, dgboost::graph_detail::iterator_stability(g_.m_vertices));
  ...
  namespace detail {
    ...
    struct adj_list_gen
    {
      ...
      struct config
      {
        ...
        typedef dgboost::detail::iterator_traits<OutEdgeIter> OutEdgeIterTraits;
        ...
        typedef dgboost::detail::iterator_traits<EdgeIter> EdgeIterTraits;
  ...
  namespace detail {
    ...
    struct adj_list_choose_vertex_pa
      : dgboost::mpl::if_<
          dgboost::is_same<Tag, vertex_all_t>,
...
namespace dgboost {} namespace boost = dgboost; namespace dgboost {
  ...
  template <typename V>
  struct hash< dgboost::detail::stored_edge<V> >
  {
    std::size_t
    operator()(const dgboost::detail::stored_edge<V>& e) const
    {
  ...
  {
    std::size_t
    operator()(const dgboost::detail::stored_edge_property<V,P>& e) const
    {
      return hash<V>()(e.m_target);
    }
  };
  ...
  {
    std::size_t
    operator()(const dgboost::detail::stored_edge_iter<V,I,P>& e) const
    {
      return hash<V>()(e.m_target);
    }
  };

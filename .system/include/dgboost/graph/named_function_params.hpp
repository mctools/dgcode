...
#define BOOST_BGL_ONE_PARAM_REF(name, key)                           \
    ...
    bgl_named_params< dgboost::reference_wrapper< PType >,             \
        BOOST_PP_CAT(key, _t), self >                                \
    name(PType& p) const                                             \
    {                                                                \
        typedef bgl_named_params< dgboost::reference_wrapper< PType >, \
            BOOST_PP_CAT(key, _t), self >                            \
            Params;                                                  \
...
namespace detail
{
    ...
    : dgboost::mpl::eval_if< dgboost::is_same< Param, param_not_found >,
          dgboost::mpl::eval_if< GraphIsConst,
    ...
    template <> struct choose_impl_helper< false >
    {
        ...
        static
            typename property_map< typename dgboost::remove_const< Graph >::type,
...
template < typename Param, typename Graph, typename PropertyTag >
typename detail::choose_impl_result< dgboost::mpl::true_, Graph, Param,
    PropertyTag >::type
choose_const_pmap(const Param& p, const Graph& g, PropertyTag tag)
{
    return detail::choose_impl_helper< !dgboost::is_same< Param,
        param_not_found >::value >::f(dgboost::mpl::true_(), g, p, tag);
}

template < typename Param, typename Graph, typename PropertyTag >
typename detail::choose_impl_result< dgboost::mpl::false_, Graph, Param,
    PropertyTag >::type
choose_pmap(const Param& p, Graph& g, PropertyTag tag)
{
    return detail::choose_impl_helper< !dgboost::is_same< Param,
        param_not_found >::value >::f(dgboost::mpl::false_(), g, p, tag);
}
...
namespace detail
{
    ...
    struct edge_capacity_value
    {
        ...
        typedef typename detail::choose_impl_result< dgboost::mpl::true_, Graph,
...
#undef BOOST_BGL_ONE_PARAM_CREF
    ...
    template < typename T > struct convert_bgl_params_to_boost_parameter
    {
        ...
        typedef dgboost::parameter::aux::tagged_argument< new_kw,
            const typename T::value_type >
            tagged_arg_type;
        typedef convert_bgl_params_to_boost_parameter< typename T::next_type >
            rest_conv;
        typedef dgboost::parameter::aux::arg_list< tagged_arg_type,
...
#define BOOST_GRAPH_DECLARE_CONVERTED_PARAMETERS(old_type, old_var)        \
    typedef typename dgboost::detail::convert_bgl_params_to_boost_parameter< \
        old_type >::type arg_pack_type;                                    \
    arg_pack_type arg_pack                                                 \
        = dgboost::detail::convert_bgl_params_to_boost_parameter<            \
...
namespace detail
{
    ...
    {
        typedef
            typename dgboost::property_map< Graph, Prop >::const_type result_type;
    ...
    {
        typedef typename dgboost::mpl::has_key< ArgPack, Tag >::type
            _parameter_exists;
    ...
    override_const_property(const ArgPack& ap,
        const dgboost::parameter::keyword< Tag >& t, const Graph& g, Prop)
    {
        typedef typename dgboost::mpl::has_key< ArgPack, Tag >::type
            _parameter_exists;
    ...
    struct override_property_t
    {
        ...
        result_type operator()(const Graph&,
            typename dgboost::add_lvalue_reference< ArgType >::type a) const
        {
            return a;
        }
    };
...
#define BOOST_GRAPH_OPENING_PART_OF_PAIR(z, i, n)                          \
    dgboost::parameter::aux::arg_list                                        \
        < dgboost::parameter::aux::tagged_argument< BOOST_PP_CAT(Keyword,    \
                                                      BOOST_PP_SUB(n, i)), \
            BOOST_PP_CAT(Arg, BOOST_PP_SUB(n, i)) >,
#define BOOST_GRAPH_MAKE_PAIR_PARAM(z, i, _)                                \
    const dgboost::parameter::aux::tagged_argument< BOOST_PP_CAT(Keyword, i), \
...
#define BOOST_GRAPH_MAKE_AP_TYPE_SPECIALIZATION(z, i, _)                  \
    ...
    {                                                                     \
        typedef BOOST_PP_REPEAT(i, BOOST_GRAPH_OPENING_PART_OF_PAIR,      \
            BOOST_PP_DEC(i)) dgboost::parameter::aux::empty_arg_list        \
...
#define BOOST_GRAPH_MAKE_FORWARDING_FUNCTION(name, nfixed, nnamed_max)        \
    ...
    template < BOOST_PP_ENUM_PARAMS(nfixed, typename Param)                   \
        ...
        typename dgboost::result_of< detail::BOOST_PP_CAT(name, _impl)          \
...
#define BOOST_GRAPH_MAKE_FORWARDING_FUNCTION_ONEX(z, nnamed, name, nfixed)     \
    ...
    typename BOOST_PP_EXPR_IF(nnamed,                                          \
        dgboost::lazy_enable_if                                                  \
            < dgboost::parameter::is_argument_pack< ArgumentPack0 >)             \
        BOOST_PP_COMMA_IF(nnamed)::dgboost::graph::detail::BOOST_PP_CAT(         \
...
#define BOOST_GRAPH_MAKE_OLD_STYLE_PARAMETER_FUNCTION(name, nfixed)            \
    template < BOOST_PP_ENUM_PARAMS(nfixed, typename Param)                    \
        ...
        class T, class R >                                                     \
    typename dgboost::result_of< ::dgboost::graph::detail::BOOST_PP_CAT(           \
        ...
        Param) BOOST_PP_EXPR_IF(nfixed, >)(BOOST_PP_ENUM_PARAMS(nfixed, Param) \
            BOOST_PP_COMMA_IF(nfixed) const typename dgboost::detail::           \
                convert_bgl_params_to_boost_parameter<                         \
                    dgboost::bgl_named_params< P, T, R > >::type&) >::type       \
    name(BOOST_PP_ENUM_BINARY_PARAMS(nfixed, const Param, &param)              \
            BOOST_PP_COMMA_IF(nfixed)                                          \
                const dgboost::bgl_named_params< P, T, R >& old_style_params)    \
    {                                                                          \
    ...
    BOOST_PP_EXPR_IF(nfixed, typename)                                         \
        dgboost::result_of< ::dgboost::graph::detail::BOOST_PP_CAT(                \
            name, _impl) BOOST_PP_EXPR_IF(nfixed,                              \
            <) BOOST_PP_ENUM_PARAMS(nfixed, Param) BOOST_PP_EXPR_IF(nfixed,    \
            >)(BOOST_PP_ENUM_PARAMS(nfixed, Param) BOOST_PP_COMMA_IF(nfixed)   \
                const dgboost::parameter::aux::empty_arg_list&) >::type          \
            name(BOOST_PP_ENUM_BINARY_PARAMS(nfixed, const Param, &param))     \
    {                                                                          \
        BOOST_GRAPH_DECLARE_CONVERTED_PARAMETERS(                              \
            dgboost::no_named_parameters, dgboost::no_named_parameters())          \
        return ::dgboost::graph::BOOST_PP_CAT(name, _with_named_params)(         \
            BOOST_PP_ENUM_PARAMS(nfixed, param) BOOST_PP_COMMA_IF(nfixed)      \
                arg_pack);                                                     \
    }
...
namespace detail
{
    ...
    {
        typedef typename dgboost::mpl::has_key< ArgPack,
            dgboost::graph::keywords::tag::vertex_index_map >::type
            _parameter_exists;
        typedef
            typename dgboost::remove_const< typename override_const_property_t<
                typename dgboost::parameter::value_type< ArgPack,
                    dgboost::graph::keywords::tag::vertex_index_map, int >::type,
                dgboost::vertex_index_t, Graph,
                _parameter_exists::value >::result_type >::type vi_map_type;
        typedef dgboost::shared_array_property_map< Value, vi_map_type > map_type;
            ...
            const Graph& g, Value v, const PM&, const ArgPack& ap)
        {
            return make_shared_array_property_map(num_vertices(g), v,
                override_const_property(ap,
                    dgboost::graph::keywords::_vertex_index_map, g,
                    vertex_index));
        }
    };
    ...
    struct map_maker
    {
        typedef typename dgboost::mpl::has_key< ArgPack, MapTag >::type
            _parameter_exists;
        BOOST_STATIC_CONSTANT(bool, has_map = (_parameter_exists::value));
        typedef map_maker_helper< has_map, Graph, ArgPack, ValueType,
            typename dgboost::remove_const< typename dgboost::parameter::value_type<
            ...
            const Graph& g, const ArgPack& ap, ValueType default_value)
        {
            return helper::make_map(g, default_value,
                ap[::dgboost::parameter::keyword< MapTag >::instance | 0], ap);
        }
    };
    ...
    static const make_property_map_from_arg_pack_gen<
        dgboost::graph::keywords::tag::color_map, default_color_type >
        ...
        KeyMapTag, IndexInHeapMapTag, Compare, Q >
    {
        ...
        typedef dgboost::d_ary_heap_indirect< ValueT, 4, index_in_heap_map,
            ...
            const Graph& g, const ArgPack& ap, KeyT defaultKey, const Q&)
        {
            return priority_queue_type(
                ...
                map_maker< Graph, ArgPack, IndexInHeapMapTag,
                    ...
                    typename dgboost::property_traits<
    ...
    struct priority_queue_maker
    {
            ...
            const Graph& g, const ArgPack& ap, KeyT defaultKey)
        {
            return helper::make_queue(g, ap, defaultKey,
                ap[::dgboost::parameter::keyword< PriorityQueueTag >::instance
                    | 0]);
        }
    };
    ...
    template < class PriorityQueueTag, class KeyT, class ValueT,
        ...
        class KeyMapTag = dgboost::graph::keywords::tag::distance_map,
        class IndexInHeapMapTag
        = dgboost::graph::keywords::tag::index_in_heap_map >
    ...
    template < typename G >
    typename dgboost::graph_traits< G >::vertex_descriptor get_null_vertex(
        const G&)
    {
        return dgboost::graph_traits< G >::null_vertex();
    }
    ...
    {
        std::pair< typename dgboost::graph_traits< G >::vertex_iterator,
            typename dgboost::graph_traits< G >::vertex_iterator >
            iters = vertices(g);
        return (iters.first == iters.second)
            ? dgboost::graph_traits< G >::null_vertex()
            : *iters.first;
    }
    ...
    {
        typedef
            typename dgboost::graph_traits< G >::vertex_descriptor result_type;
        const G& g;

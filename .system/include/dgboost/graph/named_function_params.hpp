...
#include <dgboost/core/ref.hpp>
#include <dgboost/utility/result_of.hpp>
#include <dgboost/preprocessor.hpp>
#include <dgboost/parameter/is_argument_pack.hpp>
#include <dgboost/parameter/name.hpp>
#include <dgboost/parameter/binding.hpp>
#include <dgboost/type_traits.hpp>
#include <dgboost/mpl/bool.hpp>
#include <dgboost/mpl/has_key.hpp>
#include <dgboost/graph/properties.hpp>
#include <dgboost/graph/detail/d_ary_heap.hpp>
#include <dgboost/property_map/property_map.hpp>
#include <dgboost/property_map/shared_array_property_map.hpp>
...
#define BOOST_BGL_ONE_PARAM_REF(name, key) \
    ...
    bgl_named_params<dgboost::reference_wrapper<PType>, BOOST_PP_CAT(key, _t), self> \
    name(PType& p) const { \
      typedef bgl_named_params<dgboost::reference_wrapper<PType>, BOOST_PP_CAT(key, _t), self> Params; \
      return Params(dgboost::ref(p), *this); \
    } \
...
#define BOOST_BGL_ONE_PARAM_REF(name, key) \
    template <typename PType> \
    bgl_named_params<dgboost::reference_wrapper<PType>, BOOST_PP_CAT(key, _t)> \
    name(PType& p) { \
      typedef bgl_named_params<dgboost::reference_wrapper<PType>, BOOST_PP_CAT(key, _t)> Params; \
      return Params(dgboost::ref(p)); \
    } \
...
#undef BOOST_BGL_ONE_PARAM_CREF
  ...
  namespace detail {
    ...
    template <> struct choose_impl_helper<false> {
      ...
      static typename property_map<typename dgboost::remove_const<Graph>::type, PropertyTag>::const_type
      f(dgboost::mpl::true_, const Graph& g, const Param&, PropertyTag tag) {
        return get(tag, g);
      }

      template <typename Param, typename Graph, typename PropertyTag>
      static typename property_map<typename dgboost::remove_const<Graph>::type, PropertyTag>::type
      f(dgboost::mpl::false_, Graph& g, const Param&, PropertyTag tag) {
        return get(tag, g);
      }
    };
  ...
  template <typename Param, typename Graph, typename PropertyTag>
  typename detail::choose_impl_result<dgboost::mpl::true_, Graph, Param, PropertyTag>::type
  choose_const_pmap(const Param& p, const Graph& g, PropertyTag tag)
  { 
    return detail::choose_impl_helper<!dgboost::is_same<Param, param_not_found>::value>
             ::f(dgboost::mpl::true_(), g, p, tag);
  }

  template <typename Param, typename Graph, typename PropertyTag>
  typename detail::choose_impl_result<dgboost::mpl::false_, Graph, Param, PropertyTag>::type
  choose_pmap(const Param& p, Graph& g, PropertyTag tag)
  { 
    return detail::choose_impl_helper<!dgboost::is_same<Param, param_not_found>::value>
             ::f(dgboost::mpl::false_(), g, p, tag);
  }
  ...
  namespace detail {
    ...
    struct edge_capacity_value
    {
      ...
      typedef typename detail::choose_impl_result<dgboost::mpl::true_, Graph, typename get_param_type<edge_capacity_t, Params>::type, edge_capacity_t>::type CapacityEdgeMap;
    ...
    {
      typedef bgl_named_params<P, T, R> Params;
      typedef typename detail::choose_impl_result<dgboost::mpl::true_, Graph, typename get_param_type<edge_weight_t, Params>::type, edge_weight_t>::type WeightMap;
...
#define BOOST_BGL_ONE_PARAM_REF(name, key) \
    ...
    struct convert_one_keyword<BOOST_PP_CAT(key, _t)> { \
      typedef dgboost::graph::keywords::tag::name type; \
    };
...
#undef BOOST_BGL_ONE_PARAM_CREF
    ...
    struct convert_bgl_params_to_boost_parameter {
      ...
      typedef dgboost::parameter::aux::tagged_argument<new_kw, const typename T::value_type> tagged_arg_type;
      typedef convert_bgl_params_to_boost_parameter<typename T::next_type> rest_conv;
      typedef dgboost::parameter::aux::arg_list<tagged_arg_type, typename rest_conv::type> type;
    ...
    template <>
    struct convert_bgl_params_to_boost_parameter<dgboost::no_property> {
      typedef dgboost::parameter::aux::empty_arg_list type;
      static type conv(const dgboost::no_property&) {return type();}
    };

    template <>
    struct convert_bgl_params_to_boost_parameter<dgboost::no_named_parameters> {
      typedef dgboost::parameter::aux::empty_arg_list type;
      static type conv(const dgboost::no_named_parameters&) {return type();}
    };
...
#define BOOST_GRAPH_DECLARE_CONVERTED_PARAMETERS(old_type, old_var) \
  typedef typename dgboost::detail::convert_bgl_params_to_boost_parameter<old_type>::type arg_pack_type; \
  arg_pack_type arg_pack = dgboost::detail::convert_bgl_params_to_boost_parameter<old_type>::conv(old_var);

  namespace detail {

    template <typename ArgType, typename Prop, typename Graph, bool Exists>
    struct override_const_property_t {
      typedef typename dgboost::remove_const<ArgType>::type result_type;
    ...
    struct override_const_property_t<ArgType, Prop, Graph, false> {
      typedef typename dgboost::property_map<Graph, Prop>::const_type result_type;
    ...
    struct override_const_property_result {
      typedef typename dgboost::mpl::has_key<ArgPack, Tag>::type _parameter_exists;
      typedef 
        typename override_const_property_t<
                   typename dgboost::parameter::value_type<ArgPack, Tag, int>::type,
                   Prop,
                   Graph,
    ...
    override_const_property(const ArgPack& ap, const dgboost::parameter::keyword<Tag>& t, const Graph& g, Prop) {
    typedef typename dgboost::mpl::has_key<ArgPack, Tag>::type _parameter_exists;
    ...
    struct override_property_t {
      ...
      result_type operator()(const Graph&, typename dgboost::add_lvalue_reference<ArgType>::type a) const {return a;}
    };

    template <typename ArgType, typename Prop, typename Graph>
    struct override_property_t<ArgType, Prop, Graph, false> {
      typedef typename dgboost::property_map<Graph, Prop>::type result_type;
      result_type operator()(const Graph& g, const ArgType&) const {return get(Prop(), g);}
    };
    ...
    override_property(const ArgPack& ap, const dgboost::parameter::keyword<Tag>& t, const Graph& g, Prop) {
    typedef typename dgboost::mpl::has_key<ArgPack, Tag>::type _parameter_exists;
    ...
    template <> struct make_arg_pack_type<void()> {typedef dgboost::parameter::aux::empty_arg_list type;};
    template <typename K, typename A>
    struct make_arg_pack_type<void(K, A)> {
      typedef dgboost::parameter::aux::tagged_argument<K, A> type;
    };

#define BOOST_GRAPH_OPENING_PART_OF_PAIR(z, i, n) dgboost::parameter::aux::arg_list<dgboost::parameter::aux::tagged_argument<BOOST_PP_CAT(Keyword, BOOST_PP_SUB(n, i)), BOOST_PP_CAT(Arg, BOOST_PP_SUB(n, i))>,
#define BOOST_GRAPH_MAKE_PAIR_PARAM(z, i, _) const dgboost::parameter::aux::tagged_argument<BOOST_PP_CAT(Keyword, i), BOOST_PP_CAT(Arg, i)>& BOOST_PP_CAT(kw, i)
...
#define BOOST_GRAPH_MAKE_AP_TYPE_SPECIALIZATION(z, i, _) \
    ...
    struct make_arg_pack_type<void(BOOST_PP_ENUM_PARAMS(i, Keyword), BOOST_PP_ENUM_PARAMS(i, Arg))> { \
      typedef \
        BOOST_PP_REPEAT(i, BOOST_GRAPH_OPENING_PART_OF_PAIR, BOOST_PP_DEC(i)) dgboost::parameter::aux::empty_arg_list BOOST_PP_REPEAT(i, > BOOST_PP_TUPLE_EAT(3), ~) \
        type; \
    };
...
#define BOOST_GRAPH_MAKE_FORWARDING_FUNCTION_ONEX(z, nnamed, name, nfixed) \
  ...
  > \
  typename \
  BOOST_PP_EXPR_IF(nnamed, dgboost::lazy_enable_if<dgboost::parameter::is_argument_pack<ArgumentPack0>) \
  BOOST_PP_COMMA_IF(nnamed) \
    ::dgboost::graph::detail::BOOST_PP_CAT(name, _impl)<BOOST_PP_ENUM_PARAMS_Z(z, nfixed, Param)> \
  ...
  { \
    return ::dgboost::graph::BOOST_PP_CAT(name, _with_named_params)( \
...
#define BOOST_GRAPH_MAKE_OLD_STYLE_PARAMETER_FUNCTION(name, nfixed) \
  ...
  typename dgboost::result_of< \
    ::dgboost::graph::detail::BOOST_PP_CAT(name, _impl) BOOST_PP_EXPR_IF(nfixed, <) BOOST_PP_ENUM_PARAMS(nfixed, Param) BOOST_PP_EXPR_IF(nfixed, >) \
      (BOOST_PP_ENUM_PARAMS(nfixed, Param) BOOST_PP_COMMA_IF(nfixed) \
       const typename dgboost::detail::convert_bgl_params_to_boost_parameter<dgboost::bgl_named_params<P, T, R> >::type &) \
    >::type \
  name(BOOST_PP_ENUM_BINARY_PARAMS(nfixed, const Param, & param) BOOST_PP_COMMA_IF(nfixed) const dgboost::bgl_named_params<P, T, R>& old_style_params) { \
    typedef dgboost::bgl_named_params<P, T, R> old_style_params_type; \
    BOOST_GRAPH_DECLARE_CONVERTED_PARAMETERS(old_style_params_type, old_style_params) \
    return ::dgboost::graph::BOOST_PP_CAT(name, _with_named_params)(BOOST_PP_ENUM_PARAMS(nfixed, param) BOOST_PP_COMMA_IF(nfixed) arg_pack); \
  } \
  BOOST_PP_EXPR_IF(nfixed, template <) BOOST_PP_ENUM_PARAMS(nfixed, typename Param) BOOST_PP_EXPR_IF(nfixed, >) \
  BOOST_PP_EXPR_IF(nfixed, typename) dgboost::result_of< \
    ::dgboost::graph::detail::BOOST_PP_CAT(name, _impl) BOOST_PP_EXPR_IF(nfixed, <) BOOST_PP_ENUM_PARAMS(nfixed, Param) BOOST_PP_EXPR_IF(nfixed, >) \
      (BOOST_PP_ENUM_PARAMS(nfixed, Param) BOOST_PP_COMMA_IF(nfixed) const dgboost::parameter::aux::empty_arg_list &) \
    >::type \
  name(BOOST_PP_ENUM_BINARY_PARAMS(nfixed, const Param, & param)) { \
    BOOST_GRAPH_DECLARE_CONVERTED_PARAMETERS(dgboost::no_named_parameters, dgboost::no_named_parameters()) \
    return ::dgboost::graph::BOOST_PP_CAT(name, _with_named_params)(BOOST_PP_ENUM_PARAMS(nfixed, param) BOOST_PP_COMMA_IF(nfixed) arg_pack); \
  }
  ...
  namespace detail {
    ...
    struct map_maker_helper<false, Graph, ArgPack, Value, PM> {
      typedef typename dgboost::mpl::has_key<
        ArgPack
      , dgboost::graph::keywords::tag::vertex_index_map
      >::type _parameter_exists;
      typedef typename dgboost::remove_const<
        typename override_const_property_t<
          typename dgboost::parameter::value_type<
            ArgPack, dgboost::graph::keywords::tag::vertex_index_map, int>::type,
          dgboost::vertex_index_t,
          Graph,
          _parameter_exists::value
        >::result_type>::type vi_map_type;
      typedef
        dgboost::shared_array_property_map<Value, vi_map_type>
        map_type;
      static map_type make_map(const Graph& g,
        ...
        return make_shared_array_property_map(
                 ...
                 override_const_property(
                   ap,
                   dgboost::graph::keywords::_vertex_index_map,
                   g, vertex_index));
      }
    };
    ...
    struct map_maker {
      typedef typename dgboost::mpl::has_key<ArgPack, MapTag>::type _parameter_exists;
      ...
      static map_type make_map(const Graph& g, const ArgPack& ap, ValueType default_value) {
        return helper::make_map(g, default_value, ap[::dgboost::parameter::keyword<MapTag>::instance | 0], ap);
      }
    };
    ...
    static const
      make_property_map_from_arg_pack_gen<
        dgboost::graph::keywords::tag::color_map,
        default_color_type>
    ...
    struct priority_queue_maker_helper<false, Graph, ArgPack, KeyT, ValueT, KeyMapTag, IndexInHeapMapTag, Compare, Q> {
      ...
      typedef dgboost::d_ary_heap_indirect<ValueT, 4, index_in_heap_map, typename map_maker<Graph, ArgPack, KeyMapTag, KeyT>::helper::map_type, Compare> priority_queue_type;
      ...
      make_queue(const Graph& g, const ArgPack& ap, KeyT defaultKey, const Q&) {
        return priority_queue_type(
            ...
            map_maker<Graph, ArgPack, IndexInHeapMapTag, default_index_in_heap_type>::make_map(g, ap, typename dgboost::property_traits<index_in_heap_map>::value_type(-1))
          );
      }
    };
    ...
    struct priority_queue_maker {
      typedef typename dgboost::mpl::has_key<ArgPack, PriorityQueueTag>::type _parameter_exists;
      BOOST_STATIC_CONSTANT(bool, g_hasQ = (_parameter_exists::value));
      typedef dgboost::reference_wrapper<int> int_refw;
      typedef typename dgboost::parameter::value_type<
                         ArgPack,
                         PriorityQueueTag,
                         int_refw
                       >::type
      ...
      typedef typename param_value_type_wrapper::type
        param_value_type;
      typedef typename dgboost::remove_const<param_value_type>::type param_value_type_no_const;
      ...
      static priority_queue_type make_queue(const Graph& g, const ArgPack& ap, KeyT defaultKey) {
        return helper::make_queue(g, ap, defaultKey, ap[::dgboost::parameter::keyword<PriorityQueueTag>::instance | 0]);
      }
    };

    template <class PriorityQueueTag, class KeyT, class ValueT, class Compare = std::less<KeyT>, class KeyMapTag = dgboost::graph::keywords::tag::distance_map, class IndexInHeapMapTag = dgboost::graph::keywords::tag::index_in_heap_map>
    ...
    template <typename G>
    typename dgboost::graph_traits<G>::vertex_descriptor
    get_null_vertex(const G&) {return dgboost::graph_traits<G>::null_vertex();}

    template <typename G>
    typename dgboost::graph_traits<G>::vertex_descriptor
    get_default_starting_vertex(const G& g) {
      std::pair<typename dgboost::graph_traits<G>::vertex_iterator, typename dgboost::graph_traits<G>::vertex_iterator> iters = vertices(g);
      return (iters.first == iters.second) ? dgboost::graph_traits<G>::null_vertex() : *iters.first;
    }

    template <typename G>
    struct get_default_starting_vertex_t {
      typedef typename dgboost::graph_traits<G>::vertex_descriptor result_type;
      const G& g;

...
#include <vector>
#include <dgboost/pending/queue.hpp>
#include <dgboost/graph/graph_traits.hpp>
#include <dgboost/graph/graph_concepts.hpp>
#include <dgboost/graph/visitors.hpp>
#include <dgboost/graph/named_function_params.hpp>
#include <dgboost/graph/overloading.hpp>
#include <dgboost/graph/graph_concepts.hpp>
#include <dgboost/graph/two_bit_color_map.hpp>
#include <dgboost/graph/detail/mpi_include.hpp>
#include <dgboost/concept/assert.hpp>

#include BOOST_GRAPH_MPI_INCLUDE(<dgboost/graph/distributed/concepts.hpp>)
    ...
    SourceIterator sources_end, Buffer& Q, BFSVisitor vis, ColorMap color)
{
    ...
    while (!Q.empty())
    {
        ...
        Q.pop();
        vis.examine_vertex(u, g);
        for (dgboost::tie(ei, ei_end) = out_edges(u, g); ei != ei_end; ++ei)
        {
    ...
    BFSVisitor vis, ColorMap color)
{
    ...
    typedef typename property_traits< ColorMap >::value_type ColorValue;
    typedef color_traits< ColorValue > Color;
    typename dgboost::graph_traits< VertexListGraph >::vertex_iterator i, i_end;
    for (dgboost::tie(i, i_end) = vertices(g); i != i_end; ++i)
    {
...
public:
    ...
    graph::bfs_visitor_event_not_overridden initialize_vertex(
        Vertex u, Graph& g)
    {
        invoke_visitors(m_vis, u, g, ::dgboost::on_initialize_vertex());
    ...
    {
        invoke_visitors(m_vis, u, g, ::dgboost::on_discover_vertex());
        return graph::bfs_visitor_event_not_overridden();
    }
    ...
    {
        invoke_visitors(m_vis, u, g, ::dgboost::on_examine_vertex());
        return graph::bfs_visitor_event_not_overridden();
    }
    ...
    {
        invoke_visitors(m_vis, e, g, ::dgboost::on_examine_edge());
        return graph::bfs_visitor_event_not_overridden();
    }
    ...
    {
        invoke_visitors(m_vis, e, g, ::dgboost::on_tree_edge());
        return graph::bfs_visitor_event_not_overridden();
    }
    ...
    {
        invoke_visitors(m_vis, e, g, ::dgboost::on_non_tree_edge());
        return graph::bfs_visitor_event_not_overridden();
    }
    ...
    {
        invoke_visitors(m_vis, e, g, ::dgboost::on_gray_target());
        return graph::bfs_visitor_event_not_overridden();
    }
    ...
    {
        invoke_visitors(m_vis, e, g, ::dgboost::on_black_target());
        return graph::bfs_visitor_event_not_overridden();
    }
    ...
    {
        invoke_visitors(m_vis, u, g, ::dgboost::on_finish_vertex());
        return graph::bfs_visitor_event_not_overridden();
    }
...
namespace detail
{
    ...
    void bfs_helper(VertexListGraph& g,
        ...
        const bgl_named_params< P, T, R >& params, dgboost::mpl::false_)
    {
        typedef graph_traits< VertexListGraph > Traits;
        // Buffer default
        typedef typename Traits::vertex_descriptor Vertex;
        typedef dgboost::queue< Vertex > queue_t;
        queue_t Q;
        breadth_first_search(g, s,
            choose_param(get_param(params, buffer_param_t()), dgboost::ref(Q))
                .get(),
            vis, color);
    }
    ...
    void bfs_helper(DistributedGraph& g,
        ...
        ColorMap color, BFSVisitor vis,
        const bgl_named_params< P, T, R >& params, dgboost::mpl::true_);
    ...
    template < class ColorMap > struct bfs_dispatch
    {
            ...
            const bgl_named_params< P, T, R >& params, ColorMap color)
        {
            bfs_helper(g, s, color,
                ...
                params,
                dgboost::mpl::bool_<
                    dgboost::is_base_and_derived< distributed_graph_tag,
    ...
    const bgl_named_params< P, T, R >& params)
{
    ...
    typedef dgboost::queue< vertex_descriptor > queue_t;
    queue_t Q;

    breadth_first_visit(ng, s,
        choose_param(get_param(params, buffer_param_t()), dgboost::ref(Q)).get(),
        choose_param(
...
namespace graph
{
    namespace detail
    {
        ...
        struct breadth_first_search_impl
        {
            ...
            {
                using namespace dgboost::graph::keywords;
                typename dgboost::graph_traits< Graph >::vertex_descriptor
                    sources[1]
                    = { source };
                dgboost::queue<
                    typename dgboost::graph_traits< Graph >::vertex_descriptor >
                    Q;
                dgboost::breadth_first_search(g, &sources[0], &sources[1],
                    dgboost::unwrap_ref(arg_pack[_buffer | dgboost::ref(Q)]),
                    arg_pack[_visitor | make_bfs_visitor(null_visitor())],
                    dgboost::detail::make_color_map_from_arg_pack(g, arg_pack));
            }
        };
    }
...
#include BOOST_GRAPH_MPI_INCLUDE(<dgboost/graph/distributed/breadth_first_search.hpp>)

...
#include <dgboost/pending/property.hpp> // for dgboost::lookup_one_property
#include <dgboost/pending/container_traits.hpp>
#include <dgboost/throw_exception.hpp>
#include <dgboost/tuple/tuple.hpp> // for dgboost::make_tuple
...
namespace dgboost {} namespace boost = dgboost; namespace dgboost { namespace graph {
...
public:
  ...
  {
      dgboost::throw_exception(std::runtime_error("add_vertex: "
...
{
  BOOST_STATIC_ASSERT_MSG ((dgboost::is_base_of<dgboost::graph_detail::stable_tag, VertexIterStability>::value), "Named graphs cannot use vecS as vertex container and remove vertices; the lack of vertex descriptor stability (which iterator stability is a proxy for) means that the name -> vertex mapping would need to be completely rebuilt after each deletion.  See https://svn.boost.org/trac/boost/ticket/7863 for more information and a test case.");

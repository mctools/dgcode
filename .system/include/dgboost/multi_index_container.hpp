...
#include <dgboost/mpl/deref.hpp>
#include <dgboost/multi_index_container_fwd.hpp>
#include <dgboost/multi_index/detail/access_specifier.hpp>
#include <dgboost/multi_index/detail/adl_swap.hpp>
#include <dgboost/multi_index/detail/allocator_traits.hpp>
#include <dgboost/multi_index/detail/base_type.hpp>
#include <dgboost/multi_index/detail/do_not_copy_elements_tag.hpp>
#include <dgboost/multi_index/detail/converter.hpp>
#include <dgboost/multi_index/detail/header_holder.hpp>
#include <dgboost/multi_index/detail/has_tag.hpp>
#include <dgboost/multi_index/detail/invalidate_iterators.hpp>
#include <dgboost/multi_index/detail/no_duplicate_tags.hpp>
#include <dgboost/multi_index/detail/safe_mode.hpp>
#include <dgboost/multi_index/detail/scope_guard.hpp>
...
#include <dgboost/utility/base_from_member.hpp>
...
#include <dgboost/multi_index/detail/archive_constructed.hpp>
#include <dgboost/multi_index/detail/serialization_version.hpp>
#include <dgboost/serialization/collection_size_type.hpp>
#include <dgboost/serialization/nvp.hpp>
#include <dgboost/serialization/split_member.hpp>
#include <dgboost/serialization/version.hpp>
#include <dgboost/throw_exception.hpp> 
#endif

#if defined(BOOST_MULTI_INDEX_ENABLE_INVARIANT_CHECKING)
#include <dgboost/multi_index/detail/invariant_assert.hpp>
...
class multi_index_container:
  private ::dgboost::base_from_member<
...
private:
  ...
  typedef ::dgboost::base_from_member<
...
public:
  ...
  multi_index_container(BOOST_RV_REF(multi_index_container) x):
    bfm_allocator(dgboost::move(x.bfm_allocator::member)),
    bfm_header(),
    ...
    const multi_index_container<Value,IndexSpecifierList,Allocator>& x)
  {
    ...
    swap_(y,dgboost::true_type() /* swap_allocators */);
    return *this;
  }
#endif
  ...
  {
#include <dgboost/multi_index/detail/define_if_constexpr_macro.hpp>

    BOOST_MULTI_INDEX_IF_CONSTEXPR(
      node_alloc_traits::propagate_on_container_move_assignment::value){
      swap_(x,dgboost::true_type() /* swap_allocators */);
    }
    else if(this->get_allocator()==x.get_allocator()){
      swap_(x,dgboost::false_type() /* swap_allocators */);
    }
    else{
      multi_index_container y(dgboost::move(x),this->get_allocator());
      swap_(y,dgboost::false_type() /* swap_allocators */);
    }
    return *this;

#include <dgboost/multi_index/detail/undef_if_constexpr_macro.hpp>
  }
...
BOOST_MULTI_INDEX_PROTECTED_IF_MEMBER_TEMPLATE_FRIENDS:
  ...
  void construct_value(final_node_type* x,const Value& v)
  {
    node_alloc_traits::construct(
      bfm_allocator::member,dgboost::addressof(x->value()),v);
  }

  void construct_value(final_node_type* x,BOOST_RV_REF(Value) v)
  {
    node_alloc_traits::construct(
      bfm_allocator::member,dgboost::addressof(x->value()),dgboost::move(v));
  }
  ...
  void destroy_value(final_node_type* x)
  {
    node_alloc_traits::destroy(
      bfm_allocator::member,dgboost::addressof(x->value()));
  }
  ...
  void swap_(
    multi_index_container<Value,IndexSpecifierList,Allocator>& x,
    dgboost::true_type swap_allocators)
  {
  ...
  void swap_(
    multi_index_container<Value,IndexSpecifierList,Allocator>& x,
    dgboost::false_type swap_allocators)
  {
    std::swap(bfm_header::member,x.bfm_header::member);
    super::swap_(x,swap_allocators);
    std::swap(node_count,x.node_count);
  ...
  friend class dgboost::serialization::access;
  ...
  void save(Archive& ar,const unsigned int version)const
  {
    ...
    for(iterator it=super::begin(),it_end=super::end();it!=it_end;++it){
      serialization::save_construct_data_adl(
        ar,dgboost::addressof(*it),value_version);
  ...
  void load(Archive& ar,const unsigned int version)
  {
    ...
    for(std::size_t n=0;n<s;++n){
      ...
      ar.reset_object_address(
        dgboost::addressof(p.first->value()),dgboost::addressof(value.get()));
...
>
typename ::dgboost::multi_index::index<
  ...
  multi_index_container<Value,IndexSpecifierList,Allocator>& m)BOOST_NOEXCEPT
{
  typedef multi_index_container<
    Value,IndexSpecifierList,Allocator>         multi_index_type;
  typedef typename ::dgboost::multi_index::index<
    multi_index_container<
      Value,IndexSpecifierList,Allocator>,
    Tag
  >::type                                       index_type;
...
template<
  typename Tag,typename Value,typename IndexSpecifierList,typename Allocator
>
const typename ::dgboost::multi_index::index<
  multi_index_container<Value,IndexSpecifierList,Allocator>,Tag>::type&
get(
  const multi_index_container<Value,IndexSpecifierList,Allocator>& m
)BOOST_NOEXCEPT
...
template<typename Value,typename IndexSpecifierList,typename Allocator>
struct version<
  dgboost::multi_index_container<Value,IndexSpecifierList,Allocator>
>
{
...
/* Associated global functions are promoted to namespace dgboost, except

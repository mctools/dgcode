...
#include <dgboost/smart_ptr/bad_weak_ptr.hpp>
#include <dgboost/smart_ptr/detail/sp_counted_base.hpp>
#include <dgboost/smart_ptr/detail/sp_counted_impl.hpp>
...
public:
    ...
    explicit sp_reference_wrapper( T & t): t_( dgboost::addressof( t ) )
    {
    }
...
{
    dgboost::uintptr_t v = reinterpret_cast<dgboost::uintptr_t>( p );

    // match dgboost::hash<T*>
...
public:
...
#endif
    {
        ...
        {
            dgboost::checked_delete( p );
            dgboost::throw_exception( std::bad_alloc() );
        }
    ...
    template<class Y, class D>
    explicit shared_count( dgboost::movelib::unique_ptr<Y, D> & r ): pi_( 0 )
#if defined(BOOST_SP_ENABLE_DEBUG_HOOKS)
        , id_(shared_count_id)
#endif
    {
        ...
        D2 d2( r.get_deleter() );
        pi_ = new sp_counted_impl_pd< typename dgboost::movelib::unique_ptr<Y, D>::pointer, D2 >( r.get(), d2 );
...
#endif
{
    ...
    {
        dgboost::throw_exception( dgboost::bad_weak_ptr() );
    }
}

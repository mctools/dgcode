...
#include <dgboost/core/alloc_construct.hpp>
#include <dgboost/core/first_scalar.hpp>
...
#include <dgboost/type_traits/is_bounded_array.hpp>
#include <dgboost/type_traits/is_unbounded_array.hpp>
...
struct sp_array_element {
    typedef typename dgboost::remove_cv<typename
        dgboost::remove_extent<T>::type>::type type;
};
...
struct sp_array_alignment {
    enum {
        value = sp_max_size<dgboost::alignment_of<T>::value,
            dgboost::alignment_of<U>::value>::value
    };
};
...
class sp_array_creator {
    ...
    typedef typename dgboost::type_with_alignment<sp_array_alignment<T,
...
public:
    ...
    sp_array_base(const A& other, type* start, std::size_t size)
        : state_(other, size) {
        dgboost::alloc_construct_n(state_.allocator(),
            dgboost::first_scalar(start),
    ...
    sp_array_base(const A& other, type* start, std::size_t size, const U& list)
        ...
        dgboost::alloc_construct_n(state_.allocator(),
            dgboost::first_scalar(start), state_.size() * count,
            dgboost::first_scalar(&list), count);
    }
    ...
    virtual void dispose() BOOST_SP_NOEXCEPT {
        dgboost::alloc_destroy_n(state_.allocator(),
            dgboost::first_scalar(sp_array_start<type>(this)),
            state_.size() * sp_array_count<type>::value);
    }
...
{
    return dgboost::allocate_shared<T>(dgboost::noinit_adapt(allocator), count);
}
...
{
    return dgboost::allocate_shared<T>(dgboost::noinit_adapt(allocator));
}

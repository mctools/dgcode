...
namespace dgboost {} namespace boost = dgboost; namespace dgboost{ namespace math{ namespace constants{ namespace detail{
...
inline T constant_pi<T>::compute(BOOST_MATH_EXPLICIT_TEMPLATE_TYPE_SPEC((std::integral_constant<int, N>)))
{
   ...
   T lim;
   lim = dgboost::math::tools::epsilon<T>();
   ...
   do
   {
      ...
      B = ldexp(B, 1);
      result = A - B;
      bool neg = dgboost::math::sign(result) < 0;
      if(neg)
         result = -result;
      if(result <= lim)
         break;
...
{ // http://oeis.org/A006752/constant
 ...
 //"149374281672134266498119621763019776254769479356512926115106248574");
  ...
  // 9.159655941772190150546035149323841107, 74149374281672134266498119621763019776254769479356512926115106248574422619, -01);
   ...
   return dgboost::math::constants::pi<T, dgboost::math::policies::policy<> >()
      * log(2 + dgboost::math::constants::root_three<T, dgboost::math::policies::policy<> >())
       / 8
      + 3 * sum / 8;
}
...
T khinchin(int digits)
{
   ...
   return exp(sum / dgboost::math::constants::ln_two<T, dgboost::math::policies::policy<> >());
}
...
inline T zeta_series_derivative_lead_2()
{
   ...
   return -2 * dgboost::math::constants::ln_two<T>();
}
...
inline T constant_glaisher<T>::compute(BOOST_MATH_EXPLICIT_TEMPLATE_TYPE_SPEC((std::integral_constant<int, N>)))
{
   ...
   v *= 6;
   v /= dgboost::math::constants::pi<T, forwarding_policy>() * dgboost::math::constants::pi<T, forwarding_policy>();
   v -= dgboost::math::constants::euler<T, forwarding_policy>();
   v -= log(2 * dgboost::math::constants::pi<T, forwarding_policy>());
   v /= -12;
   return exp(v);
...
{
   return 1 / dgboost::math::constants::ln_two<T>();
}
...
{
   return dgboost::math::constants::pi<T>() / 4;
}
...
{
   return 1 / dgboost::math::constants::pi<T>();
}
...
{
   return 2 * dgboost::math::constants::one_div_root_pi<T>();
}

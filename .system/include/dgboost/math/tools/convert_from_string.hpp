...
namespace dgboost {} namespace boost = dgboost; namespace dgboost{ namespace math{ namespace tools{

   template <class T>
   struct convert_from_string_result
   {
      typedef typename dgboost::conditional<dgboost::is_constructible<T, const char*>::value, const char*, T>::type type;
   };
...
#else
      return dgboost::lexical_cast<Real>(p);
#endif
   }
   template <class Real>
   ...
   template <class Real>
   BOOST_CONSTEXPR typename convert_from_string_result<Real>::type convert_from_string(const char* p) BOOST_NOEXCEPT_IF((dgboost::is_constructible<Real, const char*>::value))
   {
      return convert_from_string<Real>(p, dgboost::is_constructible<Real, const char*>());
   }

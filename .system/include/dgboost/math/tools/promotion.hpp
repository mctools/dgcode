...
#include <dgboost/type_traits/is_floating_point.hpp> // for dgboost::is_floating_point;
#include <dgboost/type_traits/is_integral.hpp> // for dgboost::is_integral
#include <dgboost/type_traits/is_convertible.hpp> // for dgboost::is_convertible
#include <dgboost/type_traits/is_same.hpp>// for dgboost::is_same
#include <dgboost/type_traits/remove_cv.hpp>// for dgboost::remove_cv
// Boost Template meta programming:
#include <dgboost/mpl/if.hpp> // for dgboost::mpl::if_c.
#include <dgboost/mpl/and.hpp> // for dgboost::mpl::if_c.
#include <dgboost/mpl/or.hpp> // for dgboost::mpl::if_c.
#include <dgboost/mpl/not.hpp> // for dgboost::mpl::if_c.
    ...
    namespace tools
    {
      ...
      { // Promote, if necessary, & pick the wider of the two floating-point types.
        ...
        typedef typename mpl::if_<
          ...
          typename mpl::if_< typename mpl::and_<mpl::not_<is_floating_point<T2P> >, ::dgboost::is_convertible<T1P, T2P> >, T2P, T1P>::type>::type type;
      }; // promote_arg2
      // These full specialisations reduce mpl::if_ usage and speed up
      // compilation:
      ...
      struct promote_args
      {
         ...
         //
         BOOST_STATIC_ASSERT_MSG((0 == ::dgboost::is_same<type, long double>::value), "Sorry, but this platform does not have sufficient long double support for the special functions to be reliably implemented.");
#endif
      };

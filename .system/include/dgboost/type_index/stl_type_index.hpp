...
/// \brief Contains dgboost::typeindex::stl_type_index class.
///
/// dgboost::typeindex::stl_type_index class can be used as a drop-in replacement 
/// for std::type_index.
///
...
/// is defined dgboost::typeindex::ctti is usually used instead of dgboost::typeindex::stl_type_index.

#include <dgboost/type_index/type_index_facade.hpp>
...
#include <dgboost/core/demangle.hpp>
...
#else
#   include <dgboost/container_hash/hash.hpp>
#endif
...
inline std::string stl_type_index::pretty_name() const {
    static const char cvr_saver_name[] = "dgboost::typeindex::detail::cvr_saver<";
    ...
    const dgboost::core::scoped_demangled_name demangled_name(data_->name());

    const char* begin = demangled_name.get();
    if (!begin) {
        dgboost::throw_exception(std::runtime_error("Type name demangling failed"));
    }
...
#else
    return dgboost::hash_range(raw_name(), raw_name() + std::strlen(raw_name()));
#endif
}
...
inline stl_type_index stl_type_index::type_id() BOOST_NOEXCEPT {
    typedef BOOST_DEDUCED_TYPENAME dgboost::remove_reference<T>::type no_ref_t;
    typedef BOOST_DEDUCED_TYPENAME dgboost::remove_cv<no_ref_t>::type no_cvr_prefinal_t;
    ...
    #  if (defined(__EDG_VERSION__) && __EDG_VERSION__ < 245) \
        ...
        typedef BOOST_DEDUCED_TYPENAME dgboost::conditional<
            dgboost::is_signed<no_cvr_prefinal_t>::value,
            dgboost::make_signed<no_cvr_prefinal_t>,
            dgboost::type_identity<no_cvr_prefinal_t>
...
inline stl_type_index stl_type_index::type_id_with_cvr() BOOST_NOEXCEPT {
    typedef BOOST_DEDUCED_TYPENAME dgboost::conditional<
        dgboost::is_reference<T>::value ||  dgboost::is_const<T>::value || dgboost::is_volatile<T>::value,
        detail::cvr_saver<T>,
        T
    >::type type;

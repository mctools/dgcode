...
protected:
    ...
    dgboost::archive::codecvt_null<typename OStream::char_type> codecvt_null_facet;
    ...
    template<class T>
    void save_impl(const T &t, dgboost::mpl::bool_<false> &){
        if(os.fail())
            dgboost::serialization::throw_exception(
                archive_exception(archive_exception::output_stream_error)
            );
    ...
    struct is_float {
        typedef typename mpl::bool_< 
            dgboost::is_floating_point<T>::value 
    ...
    template<class T>
    void save_impl(const T &t, dgboost::mpl::bool_<true> &){

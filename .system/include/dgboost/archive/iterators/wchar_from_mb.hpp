...
#include <dgboost/archive/iterators/dataflow_exception.hpp>
...
template<class Base>
class wchar_from_mb 
    : public dgboost::iterator_adaptor<
        wchar_from_mb<Base>, 
        Base, 
        wchar_t,
    ...
    >
{
    friend class dgboost::iterator_core_access;
    typedef typename dgboost::iterator_adaptor<
        wchar_from_mb<Base>, 
        Base, 
        wchar_t,
        single_pass_traversal_tag,
    ...
    dgboost::archive::detail::utf8_codecvt_facet m_codecvt_facet;
    std::mbstate_t m_mbs;

    template<typename T>
    struct sliding_buffer {
        dgboost::array<T, 32> m_buffer;
        typename dgboost::array<T, 32>::const_iterator m_next_available;
        typename dgboost::array<T, 32>::iterator m_next;
        bool m_done;
        // default ctor
        sliding_buffer() :
...
void wchar_from_mb<Base>::drain(){
    ...
    for(;;){
        typename dgboost::iterators::iterator_reference<Base>::type c = *(this->base_reference());
    ...
    const typename dgboost::iterators::iterator_value<Base>::type * input_new_start;

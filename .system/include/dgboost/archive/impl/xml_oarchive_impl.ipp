...
#include <dgboost/archive/iterators/xml_escape.hpp>
#include <dgboost/archive/iterators/ostream_iterator.hpp>

#ifndef BOOST_NO_CWCHAR
#include <dgboost/archive/wcslen.hpp>
...
void save_iterator(std::ostream &os, InputIterator begin, InputIterator end){
    typedef dgboost::archive::iterators::mb_from_wchar<
        dgboost::archive::iterators::xml_escape<InputIterator>
    > translator;
    std::copy(
        translator(begin),
        translator(end),
        dgboost::archive::iterators::ostream_iterator<char>(os)
    );
}
...
//  at least one library doesn't typedef value_type for strings
//  so rather than using string directly make a pointer iterator out of it
    typedef dgboost::archive::iterators::xml_escape<
        const char * 

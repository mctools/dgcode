...
#include <dgboost/container_hash/hash_fwd.hpp>
#include <functional>
#include <iterator>
#include <dgboost/container_hash/detail/hash_float.hpp>
#include <string>
...
#if BOOST_HASH_CXX17  // TK: OBSCURE HACK REQUESTED BY MILAN FOR CLANG15
#   define BOOST_NO_CXX98_FUNCTION_BASE  // TK: OBSCURE HACK REQUESTED BY MILAN FOR CLANG15
#endif  // TK: OBSCURE HACK REQUESTED BY MILAN FOR CLANG15
...
namespace dgboost {} namespace boost = dgboost; namespace dgboost
{
    namespace hash_detail
    {
        ...
        template <> struct basic_numbers<bool> :
            dgboost::hash_detail::enable_hash_value {};
        ...
        template <> struct long_numbers<dgboost::long_long_type> :
            dgboost::hash_detail::enable_hash_value {};
        template <> struct ulong_numbers<dgboost::ulong_long_type> :
            dgboost::hash_detail::enable_hash_value {};
#endif

#if defined(BOOST_HAS_INT128)
        template <> struct long_numbers2<dgboost::int128_type> :
            dgboost::hash_detail::enable_hash_value {};
        template <> struct ulong_numbers2<dgboost::uint128_type> :
            dgboost::hash_detail::enable_hash_value {};
#endif
    ...
    template <typename T>
    typename dgboost::hash_detail::basic_numbers<T>::type hash_value(T);
    template <typename T>
    typename dgboost::hash_detail::long_numbers<T>::type hash_value(T);
    template <typename T>
    typename dgboost::hash_detail::ulong_numbers<T>::type hash_value(T);

    template <typename T>
    typename dgboost::enable_if<dgboost::is_enum<T>, std::size_t>::type
        hash_value(T);
    ...
    template <typename T>
    typename dgboost::hash_detail::float_numbers<T>::type hash_value(T);
    ...
    namespace hash_detail
    {
        ...
        {
            inline static dgboost::uint32_t fn(dgboost::uint32_t h1, dgboost::uint32_t k1)
            {
                const dgboost::uint32_t c1 = 0xcc9e2d51;
                const dgboost::uint32_t c2 = 0x1b873593;
        ...
        {
            inline static dgboost::uint64_t fn(dgboost::uint64_t h, dgboost::uint64_t k)
            {
                const dgboost::uint64_t m = (dgboost::uint64_t(0xc6a4a793) << 32) + 0x5bd1e995;
                const int r = 47;
    ...
    template <typename T>
    typename dgboost::hash_detail::basic_numbers<T>::type hash_value(T v)
    {
        return static_cast<std::size_t>(v);
    }

    template <typename T>
    typename dgboost::hash_detail::long_numbers<T>::type hash_value(T v)
    {
        return hash_detail::hash_value_signed(v);
    }

    template <typename T>
    typename dgboost::hash_detail::ulong_numbers<T>::type hash_value(T v)
    {
...
#pragma warning(disable:4267) // 'argument' : conversion from 'size_t' to
    ...
    {
        dgboost::hash<T> hasher;
        seed = dgboost::hash_detail::hash_combine_impl<sizeof(std::size_t) * CHAR_BIT>::fn(seed, hasher(v));
    }
...
#pragma warning(pop)
    ...
    template <typename T>
    typename dgboost::hash_detail::float_numbers<T>::type hash_value(T v)
    {
        return dgboost::hash_detail::float_hash_value(v);
    }
    ...
    // dgboost::hash is defined later in extensions.hpp if
...
#define BOOST_HASH_SPECIALIZE(type) \
    template <> struct hash<type> \
         : public dgboost::hash_detail::hash_base<type> \
    { \
        std::size_t operator()(type v) const \
        { \
            return dgboost::hash_value(v); \
        } \
    };
...
#if !defined(BOOST_NO_LONG_LONG)
    BOOST_HASH_SPECIALIZE(dgboost::long_long_type)
    BOOST_HASH_SPECIALIZE(dgboost::ulong_long_type)
#endif

#if defined(BOOST_HAS_INT128)
    BOOST_HASH_SPECIALIZE(dgboost::int128_type)
    BOOST_HASH_SPECIALIZE(dgboost::uint128_type)
#endif
...
// Specializing dgboost::hash for pointers.

#if !defined(BOOST_NO_TEMPLATE_PARTIAL_SPECIALIZATION)

    template <class T>
    struct hash<T*>
        : public dgboost::hash_detail::hash_base<T*>
    {
    ...
    // dgboost::hash for all remaining types. But hash_impl is only defined
    ...
    template <class T> struct hash
        : public dgboost::hash_detail::hash_impl<dgboost::is_pointer<T>::value>
...
#include <dgboost/container_hash/extensions.hpp>
#endif

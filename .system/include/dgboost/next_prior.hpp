...
#include <dgboost/iterator/advance.hpp>
...
//    const std::list<T>::iterator prev = dgboost::prior(p);
//    const std::list<T>::iterator next = dgboost::next(prev, 2);
...
struct prior_advance_impl< T, Distance, true >
{
    static T call(T x, Distance n)
    {
        ...
        dgboost::iterators::reverse_iterator< T > rx(x);
        dgboost::iterators::advance(rx, n);
        return rx.base();
    }
};

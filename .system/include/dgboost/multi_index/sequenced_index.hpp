...
#include <dgboost/iterator/reverse_iterator.hpp>
...
public:
  ...
  typedef typename
    dgboost::reverse_iterator<iterator>            reverse_iterator;
  typedef typename
    dgboost::reverse_iterator<const_iterator>      const_reverse_iterator;
...
public:
  ...
  reverse_iterator
    rbegin()BOOST_NOEXCEPT{return dgboost::make_reverse_iterator(end());}
  const_reverse_iterator
    rbegin()const BOOST_NOEXCEPT{return dgboost::make_reverse_iterator(end());}
  reverse_iterator
    rend()BOOST_NOEXCEPT{return dgboost::make_reverse_iterator(begin());}
  const_reverse_iterator
    rend()const BOOST_NOEXCEPT{return dgboost::make_reverse_iterator(begin());}
  const_iterator
  ...
  {
    return make_iterator(node_from_value<node_type>(dgboost::addressof(x)));
  }
  ...
  std::pair<iterator,bool> push_front(BOOST_RV_REF(value_type) x)
                             {return insert(begin(),dgboost::move(x));}
  ...
  std::pair<iterator,bool> push_back(BOOST_RV_REF(value_type) x)
                             {return insert(end(),dgboost::move(x));}
  ...
  void remove(value_param_type value)
  {
    sequenced_index_remove(
      *this,
      ::dgboost::bind(std::equal_to<value_type>(),::dgboost::arg<1>(),value));
  }
...
BOOST_MULTI_INDEX_PROTECTED_IF_MEMBER_TEMPLATE_FRIENDS:
    ...
    Archive& ar,const unsigned int version,const index_loader_type& lm)
  {
    lm.load(
      ::dgboost::bind(
        &sequenced_index::rearranger,this,::dgboost::arg<1>(),::dgboost::arg<2>()),
      ar,version);
  ...
  /* This forwarding function eases things for the dgboost::mem_fn construct
...
template<typename SuperMeta,typename TagList>
inline dgboost::mpl::true_* boost_foreach_is_noncopyable(
  dgboost::multi_index::detail::sequenced_index<SuperMeta,TagList>*&,

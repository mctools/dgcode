...
#include <dgboost/multi_index/detail/copy_map.hpp>
#include <dgboost/multi_index/detail/do_not_copy_elements_tag.hpp>
#include <dgboost/multi_index/detail/index_access_sequence.hpp>
#include <dgboost/multi_index/detail/node_handle.hpp>
#include <dgboost/multi_index/detail/node_type.hpp>
...
#include <dgboost/multi_index/detail/index_loader.hpp>
#include <dgboost/multi_index/detail/index_saver.hpp>
#endif
...
protected:
  ...
  final_node_type* insert_(const value_type& v,final_node_type*& x,rvalue_tag)
  {
    x=final().allocate_node();
    BOOST_TRY{
      final().construct_value(x,dgboost::move(const_cast<value_type&>(v)));
    }
    BOOST_CATCH(...){
      final().deallocate_node(x);
      BOOST_RETHROW;
  ...
  {
    x->value()=dgboost::move(const_cast<value_type&>(v));
    return true;
  }

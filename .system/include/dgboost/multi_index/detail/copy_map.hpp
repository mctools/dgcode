...
#include <dgboost/multi_index/detail/raw_ptr.hpp>
#include <functional>
...
{
  template<typename Value>
  BOOST_RV_REF(Value) operator()(Value& x)const{return dgboost::move(x);}
};
    ...
    if(!released){
      for(size_type i=0;i<n;++i){
        alloc_traits::destroy(
          al_,dgboost::addressof((spc.data()+i)->second->value()));
  ...
  void clone(Node* node,ValueAccess access)
  {
    ...
    BOOST_TRY{
      alloc_traits::construct(
        al_,dgboost::addressof((spc.data()+n)->second->value()),
        access(node->value()));
    }
    BOOST_CATCH(...){

...
public:
  ...
  {
    if(!x.empty()){
      move_construct_allocator(dgboost::move(x));
      x.destroy_allocator();
      x.node=0;
    }
  }
      ...
      if(!empty()){
        ...
        if(!x.empty()){
          BOOST_MULTI_INDEX_IF_CONSTEXPR(
            ...
            move_assign_allocator(dgboost::move(x));
          }
          x.destroy_allocator();
        }
        else{
      ...
      alloc_traits::is_always_equal::value)
  {
    if(!empty()){
      if(!x.empty()){
        ...
        x.move_construct_allocator(dgboost::move(*this));
        destroy_allocator();
      }
    }
    else if(!x.empty()){
...
private:
  ...
  friend class dgboost::multi_index::multi_index_container;
...
#include <dgboost/multi_index/detail/ignore_wstrict_aliasing.hpp>
...
#include <dgboost/multi_index/detail/restore_wstrict_aliasing.hpp>

  void move_construct_allocator(BOOST_RV_REF(node_handle) x)
  {
    ::new (static_cast<void*>(allocator_ptr()))
      allocator_type(dgboost::move(*x.allocator_ptr()));
  }

  void move_assign_allocator(BOOST_RV_REF(node_handle) x)
  {
    *allocator_ptr()=dgboost::move(*x.allocator_ptr());
  }
  ...
  void delete_node()
  {
    ...
    alloc_traits::destroy(*allocator_ptr(),dgboost::addressof(node->value()));
...
struct insert_return_type
{
  insert_return_type(
    ...
    position(position_),inserted(inserted_),node(dgboost::move(node_)){}
  insert_return_type(BOOST_RV_REF(insert_return_type) x):
    position(x.position),inserted(x.inserted),node(dgboost::move(x.node)){}

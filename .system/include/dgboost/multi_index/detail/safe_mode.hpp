...
#include <dgboost/multi_index/detail/any_container_view.hpp>
#include <dgboost/multi_index/detail/iter_adaptor.hpp>
#include <dgboost/multi_index/safe_mode_errors.hpp>
...
#include <dgboost/detail/lightweight_mutex.hpp>
#include <dgboost/multi_index/detail/scoped_bilock.hpp>
#endif
  ...
  const Iterator1& it,const Iterator2& it0,const Iterator2& it1)
{
  if(it.valid()&&it!=it.owner()->end()&&it0.valid()){
    ...
    for(;first!=last;++first){
      ...
      if(dgboost::addressof(*first)==dgboost::addressof(*it))found=true;
    }
    if(first!=it1)return false;
    return !found;
  }
...
void transfer_equivalent_iterators(Dst&,Iterator,dgboost::true_type);
template<typename Dst,typename Iterator>
inline void transfer_equivalent_iterators(Dst&,Iterator&,dgboost::false_type);
...
BOOST_MULTI_INDEX_PRIVATE_IF_MEMBER_TEMPLATE_FRIENDS:
  ...
  friend void safe_mode_detail::transfer_equivalent_iterators(
    Dst&,Iterator,dgboost::true_type);
#endif
...
#if defined(BOOST_HAS_THREADS)
  dgboost::detail::lightweight_mutex mutex;
#endif
};
...
void safe_iterator_base::attach(safe_container_base* cont_)
{
  ...
  if(cont){
#if defined(BOOST_HAS_THREADS)
    dgboost::detail::lightweight_mutex::scoped_lock lock(cont->mutex);
#endif
...
private:
  friend class dgboost::multi_index::detail::iter_adaptor_access;
...
inline void detach_equivalent_iterators(Iterator& it)
{
  if(it.valid()){
    {
#if defined(BOOST_HAS_THREADS)
      dgboost::detail::lightweight_mutex::scoped_lock lock(it.cont->mutex);
#endif
...
inline void transfer_equivalent_iterators(Dst& dst,Iterator& i)
{
  safe_mode_detail::transfer_equivalent_iterators(
    dst,i,dgboost::is_same<Iterator,typename Dst::iterator>());
}
...
inline void transfer_equivalent_iterators(
  Dst& dst,Iterator it,dgboost::true_type /* same iterator type */)
{
  if(it.valid()){
    {
...
#if defined(BOOST_HAS_THREADS)
      detail::scoped_bilock<dgboost::detail::lightweight_mutex>
...
template<typename Dst,typename Iterator>
inline void transfer_equivalent_iterators(
  Dst&,Iterator& it,dgboost::false_type /* same iterator type */)
{
...
template<typename Iterator>
struct version<
  dgboost::multi_index::safe_mode::safe_iterator<Iterator>
>
{
  BOOST_STATIC_CONSTANT(
    int,value=dgboost::serialization::version<Iterator>::value);
};
} /* namespace serialization */
#endif

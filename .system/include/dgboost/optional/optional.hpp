...
#include <dgboost/core/swap.hpp>
#include <dgboost/optional/bad_optional_access.hpp>
...
#include <dgboost/move/utility.hpp>
#include <dgboost/none.hpp>
#include <dgboost/utility/compare_pointees.hpp>
#include <dgboost/utility/result_of.hpp>

#include <dgboost/optional/optional_fwd.hpp>
...
template <typename T>
struct optional_value_type< ::dgboost::optional<T> >
{
  typedef T type;
};

}} // namespace dgboost::optional_detail

#ifdef BOOST_OPTIONAL_CONFIG_USE_OLD_IMPLEMENTATION_OF_OPTIONAL
#include <dgboost/optional/detail/old_optional_implementation.hpp>
#else
...
class optional_base : public optional_tag
{
  ...
  protected:
    ...
    {
      if ( rhs.is_initialized() )
        construct( dgboost::move(rhs.get_impl()) );
    }
#endif
    ...
    explicit optional_base ( Expr&& expr, PtrExpr const* tag )
      ...
      m_initialized(false)
    {
      construct(dgboost::forward<Expr>(expr),tag);
    }
    ...
    BOOST_NOEXCEPT_IF(::dgboost::is_nothrow_move_constructible<T>::value && ::dgboost::is_nothrow_move_assignable<T>::value)
    {
    ...
    void assign ( optional_base&& rhs )
    {
      ...
      {
        if ( rhs.is_initialized() )
             assign_value( dgboost::move(rhs.get_impl()) );
        else destroy();
      }
      else
      {
    ...
    {
      if (is_initialized())
           assign_value( dgboost::move(val) );
      else construct( dgboost::move(val) );
    }
#endif
    ...
    {
      if (is_initialized())
        assign_expr_to_initialized(dgboost::forward<Expr>(expr),tag);
      else construct(dgboost::forward<Expr>(expr),tag);
    }
#else
    template<class Expr>
  ...
  protected :
    ...
    {
      destroy();
      construct(in_place_init, dgboost::forward<Args>(args)...);
    }
    ...
    void emplace_assign ( Arg&& arg )
     {
       destroy();
       construct(in_place_init, dgboost::forward<Arg>(arg)) ;
     }
    ...
    void construct ( Expr&& factory, in_place_factory_base const* )
     {
       dgboost_optional_detail::construct<value_type>(factory, m_storage.address());
       m_initialized = true ;
     }
    ...
    {
      assign_value( dgboost::forward<Expr>(expr) );
    }
#else
    // Constructs using any expression implicitly convertible to the single argument
    // of a one-argument T constructor.
...
struct is_optional_related
  : dgboost::conditional< dgboost::is_base_of<optional_detail::optional_tag, BOOST_DEDUCED_TYPENAME dgboost::decay<U>::type>::value
                     ...
                     || dgboost::is_same<BOOST_DEDUCED_TYPENAME dgboost::decay<U>::type, in_place_init_if_t>::value,
...
template <typename T, typename U>
struct is_optional_constructible : dgboost::is_constructible<T, U>
{};
...
template <typename T, typename U>
struct is_optional_constructible : dgboost::true_type
{};
...
struct optional_uses_direct_storage_for
  : dgboost::conditional<(dgboost::is_scalar<T>::value && !dgboost::is_const<T>::value && !dgboost::is_volatile<T>::value)
                      , dgboost::true_type, dgboost::false_type>::type
{};
...
#  define BOOST_OPTIONAL_BASE_TYPE(T) dgboost::conditional< optional_config::optional_uses_direct_storage_for<T>::value, \
  ...
  : public BOOST_OPTIONAL_BASE_TYPE(T)
{
  ...
  public :
    ...
    // Can throw if T::T(T &&) does
    optional ( rval_reference_type val ) : base(optional_detail::init_value_tag(), dgboost::forward<T>(val))
      {}
#endif
    ...
    /// Creates an optional<T> initialized with 'val' IFF cond is true, otherwise creates an uninitialized optional.
    // Can throw if T::T(T &&) does
    optional ( bool cond, rval_reference_type val ) : base( cond, dgboost::forward<T>(val) )
      {}
#endif
    ...
    explicit optional ( optional<U> const& rhs
                        ...
                        ,BOOST_DEDUCED_TYPENAME dgboost::enable_if< optional_detail::is_optional_constructible<T, U const&>, bool>::type = true
#endif
                      )
      :
      base()
...
#ifndef BOOST_OPTIONAL_DETAIL_NO_SFINAE_FRIENDLY_CONSTRUCTORS
                        ,BOOST_DEDUCED_TYPENAME dgboost::enable_if< optional_detail::is_optional_constructible<T, U>, bool>::type = true
#endif
                      )
      :
      base()
    {
      if ( rhs.is_initialized() )
        this->construct( dgboost::move(rhs.get()) );
    }
#endif
  ...
  explicit optional ( Expr&& expr,
                      BOOST_DEDUCED_TYPENAME dgboost::enable_if< optional_detail::is_optional_val_init_candidate<T, Expr>, bool>::type = true
  )
    : base(dgboost::forward<Expr>(expr),dgboost::addressof(expr))
    {}

#else
    template<class Expr>
    explicit optional ( Expr const& expr ) : base(expr,dgboost::addressof(expr)) {}
    ...
    optional& operator= ( Expr const& expr )
      {
        this->assign_expr(expr,dgboost::addressof(expr));
        return *this ;
      }
#endif // !defined  BOOST_OPTIONAL_DETAIL_NO_RVALUE_REFERENCES
    ...
    BOOST_DEDUCED_TYPENAME dgboost::enable_if<dgboost::is_same<T, BOOST_DEDUCED_TYPENAME dgboost::decay<T_>::type>, optional&>::type
    operator= ( T_&& val )
      {
        this->assign( dgboost::forward<T_>(val) ) ;
        return *this ;
      }
    ...
    {
      this->emplace_assign( dgboost::forward<Args>(args)... );
    }

    template<class... Args>
    explicit optional ( in_place_init_t, Args&&... args )
    : base( in_place_init, dgboost::forward<Args>(args)... )
    {}

    template<class... Args>
    explicit optional ( in_place_init_if_t, bool cond, Args&&... args )
    : base( in_place_init_if, cond, dgboost::forward<Args>(args)... )
    {}
    ...
    void emplace ( Arg&& arg )
     {
       this->emplace_assign( dgboost::forward<Arg>(arg) );
     }
    ...
    reference_type_of_temporary_wrapper operator *() && { return dgboost::move(this->get()) ; }
#else
    reference_const_type operator *() const { return this->get() ; }
    reference_type       operator *()       { return this->get() ; }
    ...
    template <typename F>
    optional<typename dgboost::result_of<F(reference_type)>::type> map(F f) &
      {
        if (this->has_value())
          return f(get());
        else
    ...
    template <typename F>
    optional<typename dgboost::result_of<F(reference_const_type)>::type> map(F f) const&
      {
        if (this->has_value())
          return f(get());
        else
    ...
    template <typename F>
    optional<typename dgboost::result_of<F(reference_type_of_temporary_wrapper)>::type> map(F f) &&
      {
        if (this->has_value())
          return f(dgboost::move(this->get()));
        else
          return none;
      }

    template <typename F>
    optional<typename optional_detail::optional_value_type<typename dgboost::result_of<F(reference_type)>::type>::type> flat_map(F f) &
      {
        if (this->has_value())
          return f(get());
        else
    ...
    template <typename F>
    optional<typename optional_detail::optional_value_type<typename dgboost::result_of<F(reference_const_type)>::type>::type> flat_map(F f) const&
      {
        if (this->has_value())
          return f(get());
        else
    ...
    template <typename F>
    optional<typename optional_detail::optional_value_type<typename dgboost::result_of<F(reference_type_of_temporary_wrapper)>::type>::type> flat_map(F f) &&
      {
        if (this->has_value())
          return f(dgboost::move(get()));
        else
          return none;
      }
...
# include <dgboost/optional/detail/optional_reference_spec.hpp>
#endif
...
template<class T>
inline
optional<BOOST_DEDUCED_TYPENAME dgboost::decay<T>::type> make_optional ( T && v  )
{
  return optional<BOOST_DEDUCED_TYPENAME dgboost::decay<T>::type>(dgboost::forward<T>(v));
}

// Returns optional<T>(cond,v)
template<class T>
inline
optional<BOOST_DEDUCED_TYPENAME dgboost::decay<T>::type> make_optional ( bool cond, T && v )
{
  return optional<BOOST_DEDUCED_TYPENAME dgboost::decay<T>::type>(cond,dgboost::forward<T>(v));
}
...
{
  BOOST_STATIC_ASSERT_MSG(sizeof(CharType) == 0, "If you want to output dgboost::optional, include header <dgboost/optional/optional_io.hpp>");
  return os;
}
